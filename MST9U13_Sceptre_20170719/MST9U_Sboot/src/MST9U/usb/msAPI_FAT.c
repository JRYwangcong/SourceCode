...
{
    return (File_ClusterChain xdata*)msAPI_FS_GetAddr((FILE_INODE_POOL + FILE_INODE_MEMORY_SIZE * u8FileInodeID)+(U32)0x1000);
}
...
EN_ENTRY_NEXT_RESULT FAT_DirEntryNext( U8 u8EnvironmentID, FileEntry* pFileToSet )
{
    ...
    while(TRUE)
    {
        ...
        if ( s16CurrentDirEntryIndex >= dirEntryCacheCount)
        {
            ...
            if(u8FATType==FAT_TYPE_FAT32)
            {
               if((U32)currentDirEntryIndex >= (u32EntryCountPerSector * pstFAT_FileSys->stBootSector.BPB_SecPerClus))
        ...
        else
        {
            ...
            else
            {
                ...
                FAT_DirectoryEntrySave( u8fsid, u8EnvironmentID, pstDirEntry, pFileToSet,pFAT_Env->stCurrentDirectory.u32CurrentCluster,currentDirEntryIndex);
...
//                    memcpy( (U8*)&pInode->u32DirEntryOffset, (U8*)&pFileEntry->EntryID_Ext[4], 4);
                    ...
                    memcpy(&(pFATInode->pub_inodeheader.stFileEntry), pFileEntry, sizeof(FileEntry));//pFATInode->pub_inodeheader.stFileEntry = *pFileEntry;
...
U32 FAT_FileRead(U8 u8FileHandleNo, U32 u32DesBufAdr, U32 u32ReadByteLength)
{
                                                                  ...
                                                                  u16BytePerSec,  u8FATType, u32FATTotalClusterNo, 0))
   {
    ...
    while(u32ProcessByteLength  + u16BytePerSec <= u32ReadByteLength )
    {
        ...
        if(u32ReadByteCnt < u32RemainSectorNum * u16BytePerSec)
        {
   ...
   u32FileTotalSize = pFATInode->u32FileTotalSize;
    ...
    if(bIsClusterChainActive)
    {
        ...
        for(u16Index = 0; u16Index < CLUSTER_CHAIN_SIZE; u16Index++)
        {
            ...
            if(u32TempLength== 0)
            {
               if(u16Index && u32CurrPosition == u32ClusterCount * u32BytePerClus)
               {
            ...
            if(u32CurrPosition < u32ClusterCount * u32BytePerClus)
            {
                //found!
                pFileHandle->u32FilePosition = u32CurrPosition;
...
U32 FAT_FAT32_GetUnusedSizeInKB(U8 u8fsid)
{
    ...
    while(u32CurCluster<u32MaxCluster)
    {
        ...
        if( !msAPI_MSDCtrl_SectorRead(deviceIndex, u32FATStartSectorNo + u32TempFatTableOffsetSectorNo, (U32)u16SectorSize, _PA2VA(BULK_FILE_SYSTEM_ADR)) )
        {
             __ASSERT(0);
             return 0;
        }
        ...
        {
            U32 u32Clus2Check;
            #if !(defined(MIPS_CHAKRA) || defined(ARM_CHAKRA)) //[8051+Aeon]
            pu32FatTable = (U32 xdata* )_PA2VA(msAPI_FS_GetAddr(BULK_FILE_SYSTEM_ADR+u32CurrentWindowStart));
            #else
            pu32FatTable = (U32 xdata* )_PA2VA(msAPI_FS_GetAddr(BULK_FILE_SYSTEM_ADR+u32CurrentWindowStart));
            #endif
...
U32 FAT_GetUnusedSizeInKB_New(U8 u8fsid)
{
    ...
    U32 u32FreeClusterTotalCnt,u32Cnt;
    U8 u8DeviceIndex;
    U32 u32FATStartSectorNo;
    U32 *u32pFAT= NULL;
    U32 k;
    U32 u32RecordNumClusterInOneSecter;
    U32 u32ClusterCnt=0;
...
//printf("u32FATTableOffsetSectorNo=0x%lx , FAT_startSectorNo=0x%lx, FAT size =0x%lx ,u32FATTotalClusterNo=%ld\r\n",pFAT_FileSys->u32FATTableOffsetSectorNo,pFAT_FileSys->stFAT.u32FATStartSectorNo,pFAT_FileSys->stBootSector.BPB_FATSz32,u32FATTotalClusterNo);

                u32FreeClusterTotalCnt = 0;
                ...
                for(u32Cnt = 0;u32Cnt<pFAT_FileSys->stBootSector.BPB_FATSz32;u32Cnt++)
                {
                    //printf("u32Cnt=%ld ",u32Cnt);

                    u8DeviceIndex = (U8)pFAT_FileSys->DeviceIndex;
                    u32FATStartSectorNo = pFAT_FileSys->stFAT.u32FATStartSectorNo;
                    u16BytePerSec=pFAT_FileSys->stBootSector.BPB_BytsPerSec;

                    //printf("u8DeviceIndex: %d\r\n", u8DeviceIndex);
                    //printf("u32FATStartSectorNo: %lu\r\n", u32FATStartSectorNo);
                    //printf("secter size: %d\r\n", u8SectorSize);
                    if ( !msAPI_MSDCtrl_SectorRead(u8DeviceIndex, u32FATStartSectorNo + u32Cnt,1, USB_IO_TEMP_ADDR ) )
                    {
                        //printf("*******************************1\r\n");
                        __ASSERT(0);
                        return FALSE;
                    ...
                    {
                        //printf("*******************************2  ,32RecordNumClusterInOneSecter=%ld\r\n",u32RecordNumClusterInOneSecter);
                        u32pFAT = (U32 *)msAPI_FS_GetAddr(USB_IO_TEMP_ADDR);

                        //Check number free cluster number
                        for(k=0;k<u32RecordNumClusterInOneSecter;k++)
                        {
                            ...
                            if(*u32pFAT == 0x0)
                            {
                                ...
                                //printf("(%ld) ",u32ClusterCnt);
                            }

                            //printf("*u32pFAT=0x%lx(0x%lx) ",*u32pFAT,(U32)u32pFAT);
                            u32pFAT++;
                            u32ClusterCnt++;
                        }
                ...
                freeCount=u32FreeClusterTotalCnt + 2;

                //printf("\r\nfreeClusterCount=%ld,u32ClusterCnt=%ld , u32FATTotalClusterNo=%ld\r\n",freeCount,u32ClusterCnt,u32FATTotalClusterNo);
            }
            else
            {
                freeCount=u32FATTotalClusterNo;
...
U8 FAT_GetNameByFileEntry(U8 enid, FileEntry *entry, U32 destMIUAddr,U8 charLen, BOOLEAN *bIsLongName)
{
    ...
    memcpy(&startEntry,pDirEntry,sizeof(FileEntry));
...
extern BOOLEAN FAT_DirEntryNew_Short(U8 u8fsid, U8* shortName,U8* extName, U32 u32ClusterNo, S32 s32Offset, BOOLEAN isFile);
    ...
    while(TRUE)
    {
        ...
        if((u8FATType == FAT_TYPE_FAT16 || u8FATType == FAT_TYPE_FAT12) && (u32CurrentDirStartClusterNo == u32RootDirStartClusterNo))
        {
            ...
            if(dirEntryIndexInCluster >= u16BPB_RootEntCnt)
            {
                ...
                pFAT_Env->stCurrentDirectory.s32CurrentDirEntryIndex = dirEntryIndexInCluster;
        ...
        if ( dirEntryIndexInCache >= dirEntryCacheCount)
        {
            ...
            sectorNo = FAT_FATGetSectorNo(u8fsid, u32CurrentCluster);
            sectorNo += (dirEntryIndexInCluster/dirEntryIndexInCache)*sectorPer4K;
            if ( !msAPI_MSDCtrl_SectorRead( u8DeviceIndex, sectorNo, sectorPer4K, USB_IO_TEMP_ADDR) )
            {
                __ASSERT(0);
                return ENTRY_CREATE_RESULT_ERROR;
...
U8 FAT_FileCreateDirEntry(U8 u8EnvironmentID, U16 *pu16LongName, U8 charLen, FileEntry *pEntryToSet, BOOLEAN bIsFile);
U8 FAT_FileCreateDirEntry(U8 u8EnvironmentID, U16 *pu16LongName, U8 charLen, FileEntry *pEntryToSet, BOOLEAN bIsFile)
{
    FAT_Environment* pFAT_Env;
    FAT_FileSysStruct * pFAT_FileSys;
    ...
    U8 shortName[11];
    U32 u32CurrentClusterNo, u32RootDirStartClusterNo;
    S32 lastEntryIndexInCurrentDirectory=-1;
    U8 u8FillEntryNum;
    U16 u16BPB_RootEntCnt;
    U8 u8FATType;
    BOOLEAN bIsLongName;
    ...
    FATDBG(printf("\r\n(C: %LX, P: %LX)", u32CurrentClusterNo, lastEntryIndexInCurrentDirectory););

    if((FAT_TYPE_FAT16 == u8FATType || FAT_TYPE_FAT12 == u8FATType) && (u32CurrentClusterNo == u32RootDirStartClusterNo))
    {
    ...
    if(1)//(bIsLongName)
    {
        ...
        // long-name setting
        FAT_DirEntryNew_Long(pFS_Env->u8FileSystemID, pu16LongName, charLen, u8CheckSum, (u8FillEntryNum-1), u32CurrentClusterNo, lastEntryIndexInCurrentDirectory);
        lastEntryIndexInCurrentDirectory+=(u8FillEntryNum-1);

        // short-name setting
        FAT_DirEntryNew_Short(pFS_Env->u8FileSystemID, &shortName[0],&shortName[8], u32CurrentClusterNo, lastEntryIndexInCurrentDirectory, bIsFile);

    }
    else//short name goes here
    {
...
// aaa.txt => aaa     txt
U8 FAT_FileCreate(U8 u8enid, U16 *pu16LongName, U8 charLen)
{
    BOOLEAN bResult;
    ...
    bResult = FAT_FileCreateDirEntry(u8enid, pu16LongName, charLen, &stFileEntry, TRUE);
    if(ENTRY_CREATE_RESULT_ERROR == bResult)
    {
        __ASSERT(0);
        return INVALID_FILE_HANDLE;
...
U8 FAT_FolderCreate(U8 u8enid, U16 *pu16LongName, U8 charLen)
{
    ...
    bResult = FAT_FileCreateDirEntry(u8enid, pu16LongName, charLen, &stFileEntry, FALSE);
    if(ENTRY_CREATE_RESULT_ERROR == bResult)
    {
        __ASSERT(0);
        return INVALID_FILE_HANDLE;
...
U32 FAT_FileWrite(U8 u8FileHandle, U32 u32SrcBufAddr, U32 u32BufferLen)
{
                                                                  ...
                                                                  u16BytePerSec,  u8FATType, u32FATTotalClusterNo, u32BufferLen))
   {
    ...
    __ASSERT(FAT_CLUSTER_ALLOCATED == FAT_FATGetClusterStatus( u8FATType, u32FATTotalClusterNo, pstFAT_FileHandle->u32FileCurrentClusterNo));
    __ASSERT(u8BPB_SecPerClus >= pstFAT_FileHandle->u32FileCurrentSectorNo);
    //read head sector
