...
BOOLEAN MDrv_UsbDeviceConnect(void)
{
  ...
  if (drvUSBHost_isPowerSaveModeEnable())
  {
    ...
    if (gUsbDeviceState==POWER_SAVING)
    {
...
#endif
       {
            ...
            UsbPowerSavingTimer = MDrv_GetSystemTime();
  ...
  else
  {
         ...
         (MDrv_USBGetPortAutoSwitchStatus()) )
    {
        ...
        if(MDrv_Timer_DiffTimeFromNow(UsbPortSwitchTimer) > 500)
        {
            UsbPortSwitchTimer = MDrv_GetSystemTime();
    ...
    if (MDrv_Timer_DiffTimeFromNow(UsbStartTime) > 1000 )
    {
      ...
      UsbStartTime=MDrv_GetSystemTime();
      ResetUsbHardware();
  ...
  //USB ROOT
    ...
    switch (enum_state)
    {
        ...
        case 1: // first entry
            ...
            enum_state = 2;
            usbDevEnumTimer = MDrv_GetSystemTime();
            return DEVENUM_INPROGRESS;
        case 2: // in-progress stage 1
            if (MDrv_Timer_DiffTimeFromNow(usbDevEnumTimer) < 500)
                return DEVENUM_INPROGRESS;
        ...
        case 3: // in-progress stage 2
            if (MDrv_Timer_DiffTimeFromNow(usbDevEnumTimer) < 150)
                return DEVENUM_INPROGRESS;
