...
U32 N51FS_GetTemp0XDataMIUAddr(void)
{
...
U32 N51FS_ReadMFTRecord(U8 fileSystemID,U32 MFR,U32 destMIUAddr)
{
    ...
    for(i = 1; i <= u8ReadSectorCnt; i++)
    {
        ...
        memcpy(&u16SectorLastWord,(pBuff + i*bytesPerSector - 2),2);
        if(u16SectorLastWord == u16FixUpNum)
        {
            //*(U16 *)(pBuff + i*bytesPerSector - 2) = *(U16*)(pBuff + u16OffsetToFixUp + i*2);
            memcpy((pBuff + i*bytesPerSector - 2),(pBuff + u16OffsetToFixUp + i*2),2);
        }
        else
        {
            __ASSERT(0);
...
N51FS_AttributeInfo N51FS_Attribute_Find(U8 fsid,U32 startMIUAddr,U32 destMIUAddr,EN_ATTRIBUTE_TYPE attrType, U16* pu16Offset,  U32 lowestVCN)
{
    ...
    if(attrType != ATTR_TYPE_ATTRIBUTE_LIST)
    {
        ...
        if(!info.Residented)
        {
            ...
            /*copy the data runs*/
            msAPI_FS_MIU_Copy((U32)(destMIUAddr+0x40),(U32)pu8DataRun,(info.Size-0x40));
...
static BOOLEAN  N51FS_CacheMFTData(N51FS_FileSysStruct* pFileSysToInitial)
{
    ...
    for(i = 1; i <= u8ReadSectorCnt; i++)
    {
        ...
        msAPI_FS_MIU_Copy(USB_IO_TEMP_ADDR + i*pFileSysToInitial->BytesPerSector - 2, GET_DRAM_ADDR(&u16SectorLastWord), 2);
        if(u16SectorLastWord == u16FixUpNum)
        {
            //u16OriginalWordd = *(U16*)(pBuff + i*2);
            memcpy(&u16OriginalWordd,(pBuff + i*2),2);
            msAPI_FS_MIU_Copy(GET_DRAM_ADDR(&u16OriginalWordd), USB_IO_TEMP_ADDR + i*pFileSysToInitial->BytesPerSector - 2, 2);
        }
        else
        {
            __ASSERT(0);
...
U32 N51FS_GetFreeSpaceInKB(U8 fsid)
{
    ...
    pu8Buffer = (U8 xdata*)msAPI_FS_GetAddr(USB_IO_TEMP_ADDR2);
    ...
    while(stFileEntry.FileLength.Lo > u32SizeReaded)
    {
        ...
        u32Len = msAPI_FCtrl_FileRead(u8Filehandle, _VA2PA(USB_IO_TEMP_ADDR2), u32SizeToRead);
...
U8 N51FS_FileRecordFindVolumeLabel(U8 u8DriveIndex, U16 *VolLabName, U8 VolLabNameLen)
{
    ...
    N51FS_DBG(1,printf("USB_IO_TEMP_ADDR = 0x%x\r\n\r\n",USB_IO_TEMP_ADDR));
    ...
    N51FS_DBG(1,printf("currentAddr0 = 0x%08lx\r\n",currentAddr));
    ...
    while(attrBuffer[0]!=0xFF || attrBuffer[1]!=0xFF || attrBuffer[2]!=0xFF || attrBuffer[3]!=0xFF)
    {
        ...
        currentAddr+=size;
        N51FS_DBG(1,printf("currentAddr1= 0x%08lx\r\n\r\n",currentAddr));
...
BOOLEAN N51FS_WriteMFTRecord(U8 fileSystemID,U32 MFR,U32 SrcMIUaddr)
{
    ...
    for(i = 1; i <= u8ReadSectorCnt; i++)
    {
        ...
        memcpy((pBuff + u16OffsetToFixUp + i*2),(pBuff + i*u16BytesPerSec - 2),2);
        //*(U16*)(pBuff + i*u16BytesPerSec - 2) = u16FixUpNum;
        memcpy((pBuff + i*u16BytesPerSec - 2),&u16FixUpNum,2);
    }
