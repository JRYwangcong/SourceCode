...
extern void MDrv_SysDelayUs(U32 u32InputTime);
...
void Init_BigVarInMem(void)
{
    ...
    UsbCtrlBuf = _usb_big_var_in_mem+0x00000;//malloc(0x100);
...
//                FlashProgressFuncAttach( MApp_ZUI_ACT_UpdateDownloadProgress);
    ...
    MDrv_Write4Byte(0x002C08, (  (((10) * MST_XTAL_CLOCK_HZ)) ));
    while(1);   // Necessary because of using watch dog reset
...
BOOL usb_dual_image_setting(void)
{
    ...
    g_u32DualImgOffset = ALIGN_64K(((*(U32 *)((U32)_sboot_end+0x20))+4)); // 4 bytes CRC
    bRtn = (0x00==MDrv_BDMA_CRC32(g_u32DualImgOffset, ((*(U32 *)((U32)_sboot_end+0x20+g_u32DualImgOffset))+4), BDMA_CRC32_POLY, BDMA_CRC_SEED_0, E_BDMA_SRCDEV_FLASH, FALSE));
    ...
    else
    {
        ...
        printf("CRC32 of dual image merge.bin FAIL\r\n");
    }
