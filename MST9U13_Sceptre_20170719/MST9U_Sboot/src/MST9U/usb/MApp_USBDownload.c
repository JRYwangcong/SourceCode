...
U8 MApp_CheckUsbPortStatus(void)
{
    ...
    if( (u8PortEnStatus & BIT0) == BIT0 )
    {
        ...
        while( (MDrv_Timer_DiffTimeFromNow(u32Time1) < 1000) )
        {
...
#define DOWNLOAD_BUFFER_TYPE_USB  MIU_SDRAM2SDRAM
    ...
    U8      u8FlashID = 0x0; // 0x0: flash ID0, 0x1: flash ID1
    static U8      pu8FileName[20] = "MERGE.BIN";
    U32     u32FlashSize = 0;
...
//What drivers are supposed to be disabled during FW download?
    ...
    if(!MDrv_SERFLASH_DetectSize(&u32FlashSize)) // Need refine to customer flash spec
    {
        printf("Error> Detect Flash Size Failed !!\r\n");
    }
    else
    ...
    MW_UsbDownload_Init(u8PortNo, u8FlashID, (U32)DOWNLOAD_BUFFER_ADR_USB, (U32)DOWNLOAD_BUFFER_LEN_USB, u32FlashSize, (U8)SW_UPDATE_MODE_PARTIAL_OAD, (U8)DOWNLOAD_BUFFER_TYPE_USB, pu8FileName, ZUIDrawPercentage, bSupportAPBINInFlash2);
}
