...
#define HOSTLIB_MSG(x)		x;//MS_CRITICAL_MSG(x)
...
Host20_Attach_Device_Structure    *sHubDevice;// __attribute__  ((aligned (8)));

BOOLEAN FirstBulkOut,FirstBulkIn;
//Host20_Attach_Device_Structure  xdata *psDevice_AP;
U8 *pUsbCtrlBuf;
U8 *UsbCtrlBuf;//[0x100] __attribute__ ((aligned (16)));
UINT8   *pUsbData;//[256] __attribute__ ((aligned (16))) ;
...
#define   wTimeWaitForConnect_ms 2500
...
UINT8 flib_OTGH_Init(UINT8 wForDevice_B)
{
  ...
  do {
       ...
       if (wValue==0) {
         MDrv_SysDelayUs(1000);//10, wait 1 ms
         wTimer_ms++;
       }
...
UINT8 flib_Host20_PortBusReset(void)
{
  ...
  //MsOS_DelayTask(100);
  MDrv_SysDelayUs(80000);
  UTMI_SETXBYTE(0x2a,0);
  MDrv_SysDelayUs(20000);
      ...
      // <2>.Case-1:"Setup/In/Out' Format..."
	    ...
	    if (hwDataSize>3)
	    {
	        ...
	        *dnaptr=USB_DMA_SIGNATURE1;
            ...
            {
	            dwTimes=MDrv_GetSystemTime();
	            while (*dnaptr==USB_DMA_SIGNATURE1)
	            {
	              if ((MDrv_GetSystemTime()-dwTimes) >= 10)
	              {
...
BOOLEAN USB_Hub_Handle(U8 port,U32 *devspeed)
{
  UINT8 bCMD[8];
  MDrv_SysDelayUs(10000);//10, wait 1 ms
  ...
  DEBUG_USB(printf(" Port:%x Status=",port));
  DEBUG_USB(printf("%x",(*pUsbData)));

  if(!((*pUsbData)&0x01))
    return FALSE;
  //for (i=0 ; i < 3 ; i++)
  {
    DEBUG_USB(printf("Port %x have device.\r\n",port));
	...
	if(pUsbData[1]&0x2)
	{
		DEBUG_USB(printf("\r\nLow speed device under Hub\r\n"));
		*devspeed=1;
	}
	else if(pUsbData[1]&0x4)
	{
		DEBUG_USB(printf("\r\nHigh speed device under Hub\r\n"));
		*devspeed=2;
	}
	else
	{
		DEBUG_USB(printf("\r\nFull speed device under Hub\r\n"));
		*devspeed=0;
	}
#ifdef DISABLE_LOW_SPEED_DEVICE    //avoid preamble case
    if(*devspeed==1)
    {
        DEBUG_USB(printf("NO support Low spped device while plug-in hub\r\n"));
        return FALSE;
    }
#endif
    ...
    MDrv_SysDelayUs(30000);//10, wait 1 ms
  }
  return TRUE;
}
#if 0
...
UINT8 flib_Host20_Enumerate (UINT8 bNormalEnumerate,UINT8 bAddress)
{
  ...
  DEBUG_USB(printf("Repeat FS SOF...\r\n"));
  while (1)
  {
    MDrv_SysDelayUs(100000);
  }
  ...
  MDrv_SysDelayUs(100000);            //leave some time for device to be ready
  ...
  if (sAttachDevice->sDD.bProduct != 0)
  {
	  ...
	  if(uVID==0x5E3 && uPID==0x727)
		MDrv_SysDelayUs(10000);		//alan.yu add on 3/18/2013 for special device BIZKEY
      if (flib_Host20_Issue_Control (1,bCMD,bCMD[6], pUsbData)>0)
      {
        return(0);//Fail
      }
...
Get_Str_Done:
  ...
  MDrv_SysDelayUs(150000);            //leave some time for device to be ready
...
UINT8 _flib_Host20_Enumerate (UINT8 bNormalEnumerate,UINT8 bAddress)
{
  ...
  //bReturn=1;
  bReturn=2; // set to 2 @ _flib_Host20_Enumerate
  //sAttachDevice->bISOTransferEnable=0;
   gUsbTimeout=3;
...
#if 0
  MsOS_DelayTask(150);            //leave some time for device to be ready
...
//           1:ok
//====================================================================
//extern UINT8 gPort0_PeridoicRun = 0;
...
#if (defined(CHIP_T9) || defined(CHIP_T13) || defined(CHIP_A1) || defined(CHIP_A6) || defined(CHIP_A7) || defined(CHIP_AMETHYST) || defined(CHIP_EAGLE))
