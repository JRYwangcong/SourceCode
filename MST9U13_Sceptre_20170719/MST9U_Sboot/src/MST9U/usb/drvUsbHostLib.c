...
void drvUSBHost_TurnOffPowerDownMode(void)
{
#if defined(ENABLE_USB_1) && defined(USB_POWER_SAVING_MODE)
    U32 ii;
...
void drvUSBHost_TurnOffPowerDownMode_Port2(void)
{
#if defined(ENABLE_USB_2) && defined(USB_POWER_SAVING_MODE)
    U32 ii;
...
//extern WORD msRead2Byte( DWORD u32RegLoAddr );
//extern void msWrite2Byte( DWORD u32RegLoAddr, WORD u16Value );
...
void XBYTE_OR(U32 Addr, U8 offset, U8 val)
{
    ...
    if (offset & 1)
    {
    	temp=MDrv_Read2Byte(Addr+(offset-1));
		MDrv_Write2Byte(Addr+(offset-1),(((U16)val)<<8) | (temp ));
        //temp=*(U16 volatile   *)(Addr+(offset-1)*2);
        //*(U16 volatile   *)(Addr+(offset-1)*2)=(((U16)val)<<8) | (temp );
    }
    else
    {
    	temp=MDrv_Read2Byte(Addr+offset);
		MDrv_Write2Byte(Addr+(offset),(temp )|val);
...
void XBYTE_AND(U32 Addr, U8 offset,U8 val)
{
    ...
    {
    	temp=MDrv_Read2Byte(Addr+(offset-1));
		MDrv_Write2Byte(Addr+(offset-1),((((U16)val)<<8)|0xff) & (temp ));
        //temp=*(U16 volatile   *)(Addr+(offset-1)*2);
    ...
    {
    	temp=MDrv_Read2Byte(Addr+offset);
		MDrv_Write2Byte(Addr+(offset),(temp & (0xff00|val) ));
        //temp=*(U16 volatile   *)(Addr+offset*2);
...
void XBYTE_SET(U32 Addr, U8 offset,U8 val)
{
    ...
    {
    	temp=MDrv_Read2Byte(Addr+(offset-1));
		MDrv_Write2Byte(Addr+(offset-1),((temp & 0x00ff) | (((U16)val)<<8)));
        //temp=*(U16 volatile   *)(Addr+(offset-1)*2);
...
U8 drvUSBHost_HID_Init(U8 u8UsbPort)
{
   if (u8UsbPort == USB_PORT_0)
    {
#if 1//defined(ENABLE_USB_1) && defined(USB_HID_ENABLE)
    ...
    if (u8UsbPort == USB_PORT_2)
    {
#if  defined(ENABLE_USB_2) && defined(USB2_HID_ENABLE)
