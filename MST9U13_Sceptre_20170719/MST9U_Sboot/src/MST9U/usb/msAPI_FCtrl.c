...
U8 msAPI_FCtrl_FileDelete(FileEntry* pFileEntry)
{
    ...
    if ( u8EnvironmentID == 0xFF || u8FileSysID==0xFF || pFileEntry->EntryType != ENTRY_TYPE_FILE)
    {
             __ASSERT(0);
            return FALSE;
    }
...
BOOLEAN msAPI_FCtrl_EntrySearchByShortNameFromCurrentDirEntry(U8 u8EnId,U8 *pu8FileNameToFind,U8 u8FileNameToFindLen ,FileEntry *pEntryToSet)
{
    ...
    else
    {
        ...
        S32 s32EntryIndex = 0;
        msAPI_FCtrl_EntryFirst(u8EnId);
        if (g_s32CurrentDirEntryIndex > 1)
        {
            BOOLEAN _btmp = TRUE;
            for (s32EntryIndex=1; s32EntryIndex<g_s32CurrentDirEntryIndex; s32EntryIndex++)
            {
...
/// @return -File handle number.
/***************************************************************************************/
U8 msAPI_FCtrl_FileCreate_WithoutNameChecking(U8 u8enid, U16 *pu16LongName, U8 charLen)
{
    U8 u8Result;
    U8 fsType;
    ...
    if(fsType == (U8)FILE_SYSTEM_TYPE_FAT)
    {
        u8Result = FAT_FileCreate(u8enid,pu16LongName,charLen);
    }
    else
    {
...
U8 msAPI_FCtrl_FileCreate(U8 u8enid, U16 *pu16LongName, U8 charLen, EN_CREATE_MODE createMode)
{
    ...
    return msAPI_FCtrl_FileCreate_WithoutNameChecking(u8enid,pu16LongName,charLen);
...
BOOLEAN msAPI_FCtrl_FAT16_Format(U8 driveNo)
{
    ...
    while(1)
    {
        ...
        if(MDrv_Timer_DiffTimeFromNow(_time)>3000)
        {
            FCTRL_DBG(1,printf("    .\r\n"););
            _time=MDrv_GetSystemTime();
        }
...
BOOLEAN msAPI_FCtrl_FormatPartitionByDriveIndex(EN_FORMAT_MODE formatMode,U8 u8DriveIndex)
{
    ...
    else
    {
        ...
        {
            bRet = FAT_FastFormatByDriveIndex(target_drive);
        }
