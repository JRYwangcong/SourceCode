...
#if ((ENABLE_USB_1) || (ENABLE_USB_2) || (ENABLE_CARDREADER) || (defined(ENABLE_CF)&&(ENABLE_CF == 1)) )
//status is not set
...
//extern BOOL g_bPanovelMode;
BOOL g_bPanovelMode = 0; //Supposed to be controlled by what??  leo.chou 20130904
#endif
...
#if (!BLOADER) //Bootloader
            if(g_bPanovelMode)
                MSD_DBG(printf("PanovelMode, do not do device enum\r\n"););
            else
#endif
            {
...
#if ((!ENABLE_USB_1)&&(!ENABLE_CARDREADER)&&(!ENABLE_USB_2) && !(defined(CERAMAL_N51FS) && ENABLE_CF) && !ENABLE_NAND)
