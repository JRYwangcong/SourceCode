...
static ZUI_TRANSEFFMODE _TransitionEffectOpenMapping(U32 effect_id)
{
    switch (effect_id)
    {
        ...
        case E_TRANSEFF_IN_ZOOM_IN:
            return EN_EFFMODE_PAGE_ZOOM_IN;
...
static ZUI_TRANSEFFMODE _TransitionEffectCloseMapping(U32 effect_id)
{
    switch (effect_id)
    {
        ...
        case E_TRANSEFF_OUT_ZOOM_OUT:
            return EN_EFFMODE_PAGE_ZOOM_OUT;
...
void MApp_ZUI_ACT_TransitionEffectBegin(ZUI_TRANSEFFMODE mode, ZUI_STATE target)
{
    ...
    _u32TransEffectMonitorTimer = MsOS_GetSystemTime();

    _eTransEffectMode = mode;
    _eZUIState = E_ZUI_STATE_TRANSEFF_DOING;

    #if ZUI_EFFECT_DBG_TIME_DIFF
    _u32MainMenuPrevTime = MsOS_GetSystemTime();
    #endif
    ...
    switch( _eTransEffectMode )
    {
        ...
        case EN_EFFMODE_MAIN_BG_ROTATE:
            ...
            _u32TransEffectTimer = MsOS_GetSystemTime();
            break;
...
void MApp_ZUI_ACT_TransitionEffectDoing(void)
{
    ...
    ZUI_MSG(printf("AEON busy! timer=%ld %d\n",MsOS_Timer_DiffTimeFromNow(_u32TransEffectMonitorTimer),_u8TransitionStep));
    if (MsOS_Timer_DiffTimeFromNow(_u32TransEffectMonitorTimer) > 220
    && (_eTransEffectMode == EN_EFFMODE_PAGE_FADE_IN)
    )
    {
        _u8TransitionStep = 1;
    }
    if (MsOS_Timer_DiffTimeFromNow(_u32TransEffectMonitorTimer) > 110
    ...
    switch( _eTransEffectMode )
    {
        ...
        #if(!TRUNK_UI2)
        {
            ...
            #if ZUI_EFFECT_DBG_TIME_DIFF
            printf("time_diff=%lu\n", MsOS_Timer_DiffTimeFromNow(_u32MainMenuPrevTime));
            _u32MainMenuPrevTime = MsOS_GetSystemTime();
            #endif //ZUI_EFFECT_DBG_TIME_DIFF
            ...
            {
                if (MsOS_Timer_DiffTimeFromNow(_u32TransEffectTimer)<30)
                {
                    break;
                }
            }
            ...
            {
                if (MsOS_Timer_DiffTimeFromNow(_u32TransEffectTimer)<(U32)((_u8TransitionStep)*10))
                {
                    break;
                }
        ...
        {
            //speed control....
            if (MsOS_Timer_DiffTimeFromNow(_u32TransEffectTimer)<20)
            {
                break;
            }
            _u32TransEffectTimer = MsOS_GetSystemTime();
        ...
        case EN_EFFMODE_PAGE_ZOOM_IN:
        {
            ...
            _u8TransitionStep--;
            {
                ...
                if (_u8TransitionStep == (MSGBOX_ZOOM_IN_STEP-1))
                {
                    ...
                    // clear frame buffer
                    MApi_GOP_GWIN_Switch2FB(u8FbIdTmp);
        ...
        case EN_EFFMODE_PAGE_ZOOM_OUT:
        {
            ...
            else
            {
                ...
                MApp_ZUI_API_ClearDC(pBuffDC, MPLAYER_TRANSPARENT_COLOR);
