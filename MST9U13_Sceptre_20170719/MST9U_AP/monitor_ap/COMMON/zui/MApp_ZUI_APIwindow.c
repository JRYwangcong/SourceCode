...
#define ZUI_VARDATA_MAXBYTESIZE    (2000/32)  // (1300/32) //(1100/32) //for 800 windows...

//ZUI_MAX_MOVABLE_HWND: number of hwnd can be moved and scaled
#define ZUI_MAX_MOVABLE_HWND          2000 // 1300// 1100
...
void MApp_ZUI_API_EmptyMessageQueue (void)
{
    ...
    _MenuMsgQueue.u32PrevTimerEntered = MsOS_GetSystemTime();
...
BOOLEAN MApp_ZUI_API_SetTimer (HWND hWnd, U32 u32id, U32 period)
{
    ...
    //printf("\n[WndSetTimer]  wnd = %d, id = %d, period = %ld", hWnd, u8id, period);
...
BOOLEAN _MApp_ZUI_API_WindowProcOnTimer(void)
{
    ...
    */
    diff_time = MsOS_Timer_DiffTimeFromNow(_MenuMsgQueue.u32PrevTimerEntered);
    _MenuMsgQueue.u32PrevTimerEntered = MsOS_GetSystemTime();
    ...
    switch (_eZUIState)
    {
        ...
        case E_ZUI_STATE_RUNNING:
            ...
            if (_MenuMsgQueue.MsgQueue[_MenuMsgQueue.u32MsgQIdxEnd].hwnd != HWND_INVALID)
                return TRUE;
            if (MsOS_Timer_DiffTimeFromNow(_MenuMsgQueue.u32PrevTimerEntered) > ZUI_LOW_FREQ_TIMER_THRESHOLD)
                return TRUE;
            break;
...
BOOLEAN _MApp_ZUI_API_WindowProcOnIdle(void)
{
    ...
    if (_InvClip.bNeedRepaint)
    {
        ...
        #ifdef ZUI_REPAINT_TIME_DEBUG_THRESHOLD
        if (ZUI_REPAINT_TIME_DEBUG_THRESHOLD <= MsOS_Timer_DiffTimeFromNow(now))
        {
