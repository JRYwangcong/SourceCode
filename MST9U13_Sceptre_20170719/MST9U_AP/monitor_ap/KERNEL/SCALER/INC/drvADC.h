...
#define ADC_MID(a,b,c) (a<b ? (b<c ? b : a<c ? c: a) : (b>c ? b : a>c ? c:a) ) 
...
INTERFACE void msADC_init(void);
INTERFACE void msADC_SetConfiguration(BYTE ucDisplayEn);
INTERFACE void drvADC_init(Bool IsShareGrd);
INTERFACE void drvADC_SetInputMux(BYTE port_type );
...
INTERFACE void drvADC_ADCLinearityOffsetGainMismatchCal(void);
#else
...
INTERFACE Bool drvADC_AutoAdcColor(BYTE u8VSyncTime, BYTE SCDetWin);
...
INTERFACE BYTE drvADC_WaitAutoStatusReady(DWORD regAddr, BYTE regFlag);
...
INTERFACE void drvADC_AdjustCalDelay(WORD u8CalDelay);
