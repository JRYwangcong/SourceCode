...
#define OSD_WND_INFO_H_DUPE_RATIO   6
#define OSD_WND_INFO_V_DUPE_RATIO   7
...
#define OSD_PG_RED					0
#define OSD_PG_GREEN				1
#define OSD_PG_BLUE					2	
...
#define GET_CABASE_0_90(u16Base,u8LineOffset,u8H)      	(u16Base+(WORD)(u8LineOffset)*(u8H-1)) 
#define GET_CABASE_90_0(u16Base,u8LineOffset,u8H)       (u16Base-(WORD)(u8LineOffset)*(u8H-1))
...
#define GET_CABASE_0_270(u16Base,u8LineOffset)      	(u16Base+u8LineOffset-1)
#define GET_CABASE_270_0(u16Base,u8LineOffset)      	(u16Base+1-u8LineOffset)
...
typedef struct
{
    ...
    BYTE u8DeltaInit:     8;
    BYTE u8Step:          8;
    Bool bDataSign:       1; //0:+, 1:-
    Bool bDeltaSign:      1; //0:+, 1:-
    BYTE u8Delta:         6;
    WORD u16Length0:      11;
    WORD u16Length1:      11;
    Bool bOffsetSign:     1; //0:+, 1:-
    Bool bOverflowEnable: 1;
    Bool bDotEnable:      1;
    BYTE u8CurveMode:     2;
}
CurveClass;
...
INTERFACE void drvOSD_Init(void);
...
INTERFACE void drvOSD_SetWndShiftPixel(BYTE u8Wnd,BYTE u8X,BYTE u8Y);
INTERFACE WORD drvOSD_GetWndInfo(BYTE u8Wnd,BYTE u8Dir);
INTERFACE void drvOSD_SetGDEngineEnableWnd(BYTE u8GDEngine, WORD u16EnableFlags,WORD u16Mask);

INTERFACE void drvOSD_SetTGenHPosition(BYTE u8Tgen, WORD u16HStart, WORD u16HEnd);
INTERFACE void drvOSD_SetTGenVPosition(BYTE u8Tgen, WORD u16VStart, WORD u16VEnd);
INTERFACE void drvOSD_SetTGenFixColor(BYTE u8Tgen, BYTE u8RColor, BYTE u8GColor, BYTE u8BColor);
INTERFACE void drvOSD_SetCurveClass(BYTE u8CurveIndex, const CurveClass* pstHCurve, const CurveClass* pstVCurve);
INTERFACE void drvOSD_SetTgenCurve(BYTE u8Tgen, BYTE u8ARGB, BYTE u8Engine, BYTE u8CurveIndex, const CurveClass* pstHCurve, const CurveClass* pstVCurve);
INTERFACE void drvOSD_SetTGenOn(BYTE u8Tgen,Bool u8On);
INTERFACE void drvOSDSetMask0(BYTE u8MaskKeyIndex, BYTE u8OnOff);
INTERFACE void drvOSD_ClrOSD_SRAM(void);

#if (CHIP_ID == MST9U3)
#define OSD_COMMON_WRITE_ONOFF(ON_OFF)	(msWriteByteMask(REG_101C00, ON_OFF << 7, BIT7))
#endif

// These functions are reserved for setting L or R OSD gradient windows respectively 
#if (CHIP_ID == MST9U3 && ENABLE_OSD_LR)
INTERFACE void drvOSD_SetCurveClassR(BYTE u8CurveIndex, const CurveClass* pstHCurve, const CurveClass* pstVCurve);
INTERFACE void drvOSD_SetTGenHPositionL(BYTE u8Tgen, WORD u16HStart, WORD u16HEnd);
INTERFACE void drvOSD_SetTGenHPositionR(BYTE u8Tgen, WORD u16HStart, WORD u16HEnd);
INTERFACE void drvOSD_SetTGenVPositionL(BYTE u8Tgen, WORD u16VStart, WORD u16VEnd);
INTERFACE void drvOSD_SetTGenVPositionR(BYTE u8Tgen, WORD u16VStart, WORD u16VEnd);
INTERFACE void drvOSD_SetTgenCurveL(BYTE u8Tgen, BYTE u8ARGB, BYTE u8Engine, BYTE u8CurveIndex, const CurveClass* pstHCurve, const CurveClass* pstVCurve);
INTERFACE void drvOSD_SetTgenCurveR(BYTE u8Tgen, BYTE u8ARGB, BYTE u8Engine, BYTE u8CurveIndex, const CurveClass* pstHCurve, const CurveClass* pstVCurve);
INTERFACE void drvOSD_SetTGenOnL(BYTE u8Tgen,Bool u8On);
INTERFACE void drvOSD_SetTGenOnR(BYTE u8Tgen,Bool u8On);
#endif
