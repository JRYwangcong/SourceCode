...
#if ADC_DEBUG & ENABLE_MSTV_UART_DEBUG
...
//75% SMPTE
#define Y_MAX_VALUE             (235*0.75+4)*4
#define Y_MIN_VALUE             (16*0.75+4)*4
#define PbPr_MAX_VALUE          (240*0.75+32)*4
#define PbPr_MIN_VALUE          (16*0.75+32)*4
...
static Bool drvADC_AutoAdjustYUVAdcGain(BYTE vsyncTime, DWORD u32SCRegBase);
static Bool drvADC_AutoAdjustYUVAdcOffset(BYTE vsyncTime, DWORD u32SCRegBase);
...
Bool drvADC_AutoAdcColor(BYTE u8VSyncTime, BYTE InputPort)
{
    ...
    if(USER_PREF_INPUT_COLOR_FORMAT(0)==INPUT_COLOR_YPBPR) //YPbPr or YUV_H+V
    ...
    {
        if (drvADC_AutoAdjustYUVAdcGain(u8VSyncTime, u32SCRegBase))
            result = drvADC_AutoAdjustYUVAdcOffset(u8VSyncTime, u32SCRegBase);
    }
    else
...
//**************************************************************************
//  [Function Name]:
//                  drvADC_AdjustCalDelay()
//  [Description]
//                  Adjust ADC delay for specfic SOG(VESA) timing
//  [Arguments]:
//                  u8CalDur: pixel
//  [Return]:
//
//**************************************************************************
void drvADC_AdjustCalDelay(WORD u8CalDelay)
{
    drvADC_Write2BytesMask(REG_ADC_DTOP_17_L, u8CalDelay, 0x0FFF);
}
...
#if ENABLE_ADC_RESET
    // ADC software reset for 高溫白屏issue
...
static void drvADC_LoadTable(TAB_Info* pTab_info)
{
    ...
    {
         u32Addr =  (DWORD)( (pTab_info->pTable[0]<<16) + (pTab_info->pTable[1]<<8) + pTab_info->pTable[2] );
...
//                  TRUE: done, FALSE: fail
//**************************************************************************
static Bool drvADC_AutoAdjustYUVAdcGain(BYTE u8VSyncTime, DWORD u32SCRegBase)
{
    ...
    msWriteByteMask(u32SCRegBase+SC01_53, BIT0, BIT0); // h29[8] Auto Range disable
    msWrite2Byte(u32SCRegBase+SC01_58, ADC_Range); // h2B V length
    msWrite2Byte(u32SCRegBase+SC01_5A, ADC_Range); // h2C H length
#if ADC_Range_Debug
    msWrite2Byte(u32SCRegBase+SC01_C0, 0xC210); // h60[15] Debug mode enable, [14:10]:r; [9:5]:g; [4:0]:b
#endif
    area_shift = msRead2Byte(u32SCRegBase+SC01_0E)/7/2;
    ...
    for (ucTmp=0; ucTmp<0xFF; ucTmp++)
    {
        ...
        for( i=0; i < 5; i++)
        {
            // Get min
            // Y
            msWrite2Byte(u32SCRegBase+SC01_54, msRead2Byte(u32SCRegBase+SC01_08)+msRead2Byte(u32SCRegBase+SC01_0C)*7/8); // h2A V start
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift*13); // h2B H start //black
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x01, 0x07);
            Delay1ms(u8VSyncTime);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);
            minG[i] =  msRead2Byte(u32SCRegBase+SC01_22);

            msWrite2Byte(u32SCRegBase+SC01_54, msRead2Byte(u32SCRegBase+SC01_08)+msRead2Byte(u32SCRegBase+SC01_0C)/3); // h2A V start

            // Pb
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift*3); //yellow
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x02, 0x07);
            Delay1ms(u8VSyncTime);
            minB[i] =  msRead2Byte(u32SCRegBase+SC01_22);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);

            // Pr
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift*5); //cyan
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x00, 0x07);
            Delay1ms(u8VSyncTime);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);
            minR[i] =  msRead2Byte(u32SCRegBase+SC01_22);

            // Get Max
            //  Y
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift); //white
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x04, 0x07);
            Delay1ms(u8VSyncTime);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);
            maxG[i] = msRead2Byte(u32SCRegBase+SC01_22);

          //  Pb
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift*13); //blue
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x05, 0x07);
            Delay1ms(u8VSyncTime);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);
            maxB[i] = msRead2Byte(u32SCRegBase+SC01_22);

            //  Pr
            msWrite2Byte(u32SCRegBase+SC01_56,  msRead2Byte(u32SCRegBase+SC01_0A)+area_shift*11); //red
            msWriteByteMask(u32SCRegBase+SC01_1E, 0x03, 0x07);
            Delay1ms(u8VSyncTime);
            drvADC_WaitAutoStatusReady(u32SCRegBase+SC01_1C, BIT1);
            maxR[i] = msRead2Byte(u32SCRegBase+SC01_22);
        }
        ...
        if((ResultR>=PbPr_AUTO_ACTIVE_RANGE)&&(ResultR<=(PbPr_AUTO_ACTIVE_RANGE+1)))  // Range: 0x10~0xF0
        {
            adcStatus |= _BIT4;
        }
        else
        ...
        if((ResultG>=Y_AUTO_ACTIVE_RANGE)&&(ResultG<=(Y_AUTO_ACTIVE_RANGE+1)))  // Range: 0x00~0xDB
        {
            adcStatus |= _BIT3;
        }
        else
        ...
        if (!(atgFlags&_BIT1))
        {
            ...
            {
                rGain += (signed)(PbPr_AUTO_ACTIVE_RANGE - ResultR)*ADC_Tune_Step;
                atgFlags &= ~_BIT1;
            }
        }
        ...
        if (!(atgFlags&_BIT3))
        {
            ...
            {
                gGain += (signed)(Y_AUTO_ACTIVE_RANGE - ResultG)*ADC_Tune_Step;
                atgFlags &= ~_BIT3;
            }
        }
        ...
        if (!(atgFlags&_BIT5))
        {
            ...
            {
                bGain += (signed)(PbPr_AUTO_ACTIVE_RANGE - ResultB)*ADC_Tune_Step;
                atgFlags &= ~_BIT5;
            }
        }
        ...
        {
#if ADC_Range_Debug
            msWriteByteMask(u32SCRegBase+SC01_C1, 0, BIT7); // h60[15] Debug mode disable
#endif
            msWriteByteMask(u32SCRegBase+SC01_53, 0, BIT0); // h29[8] Auto Range enable
            return FALSE;
        }
        }
...
//                  TRUE: done, FALSE: fail
//**************************************************************************
static Bool drvADC_AutoAdjustYUVAdcOffset(BYTE u8VSyncTime, DWORD u32SCRegBase)
{
    ...
    for (ucTmp=0; ucTmp<0xFF; ucTmp++)
    {
        ...
        if((ResultR>=PbPr_MIN_VALUE)&&(ResultR<=(PbPr_MIN_VALUE+1)))  // Range: 0x10~0xF0
        {
            adcStatus |= _BIT4;
        }
        else
        ...
        if((ResultG>=Y_MIN_VALUE)&&(ResultG<=(Y_MIN_VALUE+1)))  // Range: 0x00~0xDB
        {
            adcStatus |= _BIT3;
        }
        else
        ...
        if (!(atgFlags&_BIT1))
        {
            ...
            {
                rOffset += (signed)(PbPr_MIN_VALUE - ResultR)*ADC_Tune_Step;
                atgFlags &= ~_BIT1;
            }
        }
        ...
        if (!(atgFlags&_BIT3))
        {
            ...
            {
                gOffset += (signed)(Y_MIN_VALUE - ResultG)*ADC_Tune_Step;
                atgFlags &= ~_BIT3;
            }
        }
        ...
        if (!(atgFlags&_BIT5))
        {
            ...
            {
                bOffset += (signed)(PbPr_MIN_VALUE - ResultB)*ADC_Tune_Step;
                atgFlags &= ~_BIT5;
            }
        }
    ...
    if(atgFlags ==(_BIT1|_BIT3|_BIT5))
    {
        USER_PREF_ADC_RED_OFFSET  = rOffset;
        USER_PREF_ADC_GREEN_OFFSET = gOffset;
        USER_PREF_ADC_BLUE_OFFSET = bOffset;
        drvADC_SetRGBOffsetCode(rOffset, gOffset, bOffset);
        msWriteByteMask(u32SCRegBase+SC01_53, 0, BIT0); // h29[8] Auto Range enable
        return TRUE;
    }
...
static void drvADC_EnableScalerPG(msADCScalerPG_HtotalSel ht_sel)
{
    ...
    msWrite2ByteMask(SC01_0A, 0x0010, SC_MASK_IP1_CapHST); // SPRHST = 0x10
    whact = 256*(ht_sel+1) - 16;
    msWrite2ByteMask(SC01_0E, whact,SC_MASK_IP1_CapHDC); // SPRHDC = htsel - 0x10
    // Vtotal is 64 lines, Vstart = 0x10, Vactive = 0x30
    msWrite2ByteMask(SC01_08, 0x0010, SC_MASK_IP1_CapVST); // SPRVST = 0x10
    msWrite2ByteMask(SC01_0C, 0x0030,SC_MASK_IP1_CapVDC); // SPRVDC = 0x30
    ...
    msWriteByteMask(REG_2E21, BIT7, BIT7); // REG_2E10[15]: Data切成ADC data
}
...
static void drvADC_BackupScalerSetting(void)
{
    ...
    gScalerBackupSetting.SPRHST = msRead2Byte( SC01_0A ) & SC_MASK_IP1_CapHST; //???? 0xFFF
    gScalerBackupSetting.SPRHDC = msRead2Byte( SC01_0E ) & SC_MASK_IP1_CapHDC;
    gScalerBackupSetting.SPRVST = msRead2Byte( SC01_08 ) & SC_MASK_IP1_CapVST; //???? 0xFFF
    gScalerBackupSetting.SPRVDC = msRead2Byte( SC01_0C ) & SC_MASK_IP1_CapVDC;
}
...
static void drvADC_RecoverScalerSetting(void)
{
    ...
    msWrite2ByteMask( SC01_0A, gScalerBackupSetting.SPRHST, SC_MASK_IP1_CapHST ); //????
    msWrite2ByteMask( SC01_0E, gScalerBackupSetting.SPRHDC, SC_MASK_IP1_CapHDC );
    msWrite2ByteMask( SC01_08, gScalerBackupSetting.SPRVST, SC_MASK_IP1_CapVST );  //????
    msWrite2ByteMask( SC01_0C, gScalerBackupSetting.SPRVDC, SC_MASK_IP1_CapVDC );
    ...
    msWriteByteMask(REG_2E21, 0, BIT7); // REG_2E10[15]: Data切成ADC data
}
