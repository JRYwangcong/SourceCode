...
#define MApi_XC_R2BYTE(u32Reg)	\
    msRead2Byte(0x80000000 + (((MS_U16)u32Reg & 0xFF00)<<16) + _baseAddr+ ((MS_U8)u32Reg & 0xFF))

#define MApi_XC_W2BYTEMSK(u32Reg, u16Val, u16mask)  \
    msWrite2ByteMask(0x80000000 + (((MS_U16)u32Reg & 0xFF00)<<16) + _baseAddr + (u32Reg & 0xFF), u16Val, u16mask)

#define MApi_XC_W2BYTE(u32Reg, u16Val)  \
    msWrite2Byte(0x80000000 + (((MS_U16)u32Reg & 0xFF00)<<16) + _baseAddr + (u32Reg & 0xFF), u16Val)

#define MApi_XC_R2BYTEMSK(u32Reg, u16mask) \
    (msRead2Byte(0x80000000 + (((MS_U16)u32Reg & 0xFF00)<<16) + _baseAddr + (u32Reg & 0xFF)) & (u16mask))
...
static unsigned long   _baseAddr = 0x102F00;
static BYTE    _WinIdx = 0;

//-------------------------------------------------------------------------------------------------
//  Local Variables
//-------------------------------------------------------------------------------------------------
static SC_ApiStatus _status;
...
//  Local Functions
//-------------------------------------------------------------------------------------------------
MS_U32 _Read2BYTEMASK(MS_U32 reg, MS_U16 mask, BOOL bMaskIn)
{
    WORD u16ScalerMask;
    if (bMaskIn)
        u16ScalerMask = g_DisplayWindow[_WinIdx].u16DwScMaskIn;
    else
        u16ScalerMask = g_DisplayWindow[_WinIdx].u16DwScMaskOut;
    BYTE u8ScIdx;
    unsigned long   baseAddr_bk = _baseAddr;
    ...
    do
    {
        if (u16ScalerMask & _BIT0)
        {
            ...
            _baseAddr = baseAddr;
            u32Ret+= (MS_U32)MApi_XC_R2BYTEMSK(reg, mask);
        }
        u16ScalerMask >>= 1;
        u8ScIdx++;
    }while (u16ScalerMask > 0);
    _baseAddr = baseAddr_bk;
    return u32Ret;

}

void _Write2BYTEMASK(MS_U32 reg, MS_U16 mask, BOOL bMaskIn, MS_U32 value)
{
    WORD u16ScalerMask;
    if (bMaskIn)
        u16ScalerMask = g_DisplayWindow[_WinIdx].u16DwScMaskIn;
    ...
    do
    {
        if (u16ScalerMask & _BIT0)
        {
            baseAddr = SCRegBase(u8ScIdx);
            _baseAddr = baseAddr;
            MApi_XC_W2BYTEMSK(reg,value, mask);
        }
        u16ScalerMask >>= 1;
        u8ScIdx++;
    }while (u16ScalerMask > 0);
...
/******************************************************************************/
MS_U16 MDrv_SC_get_adaptive_ctrl(void)
{
    MS_U16 u16Ctrl;

    u16Ctrl = MApi_XC_R2BYTEMSK(REG_SC_BK22_7C_L, 0xFF);
...
MS_U32 MDrv_SC_read_motion_value2(void)
{
    ...
    u32MotionValue = _Read2BYTEMASK(REG_SC_BK0A_18_L, 0xFFF, TRUE) + _Read2BYTEMASK(REG_SC_BK0A_1A_L, 0x7F, TRUE);
...
void MDrv_SC_mcnr_driver( MS_U32 nonHistoryMotionPixel )
{
    ...
    is24to60Hz = (!_status.bInterlace) && (_status.u16InputVFreq < 300);
...
/******************************************************************************/
///Calculate and set its weight
///@param bBouncing_status \     IN: bounceins status
/******************************************************************************/

#define SPF_OP1_HisWeight_MaxValue 0x06    // history weight = 6
#define SPF_OP1_HisWeight_MinValue 0x00    // history weight = 2
...
void MDrv_SC_SPF_OP1_HisWeight_Ctrl(const MS_BOOL Bouncing_status)
{
    ...
    if( (Bouncing_status && !film) )
    {
        reg_uc_his_weight = SPF_OP1_HisWeight_MinValue; //still
        //_Write2BYTEMASK(REG_SC_BK22_01_L, 0x3F00, FALSE, 0x3800);
    }
    else
    {
        if( reg_uc_his_weight < SPF_OP1_HisWeight_MaxValue ) //refer to the history
            reg_uc_his_weight++;
        else
            reg_uc_his_weight = SPF_OP1_HisWeight_MaxValue;

        if ( g_u8Ymax[_WinIdx] == g_u8Ymin[_WinIdx])    //for pattern generator changing pattern!
        {
            _Write2BYTEMASK(REG_SC_BK22_01_L, 0x3F00, FALSE, 0x3800);
            reg_uc_his_weight = SPF_OP1_HisWeight_MinValue; //still
            u8Cnt = 3;
        ...
        else
        {
            ...
            {
            #if (CHIP_ID == MST9U3)
                _Write2BYTEMASK(REG_SC_BK22_01_L, 0x3F00, FALSE, 0x0400);
            #else
                _Write2BYTEMASK(REG_SC_BK22_01_L, 0x3F00, FALSE, 0x1400);
            #endif
    ...
    //MApi_XC_W2BYTEMSK(REG_SC_BK22_0A_L, reg_uc_his_weight, 0x0007);
    _Write2BYTEMASK(REG_SC_BK22_0A_L, 0x0007, FALSE, reg_uc_his_weight);
}
...
void MDrv_SC_de_bouncing(MS_U32 u32MotionValue)
{
    ...
    MS_BOOL Bouncing_status;

    Bouncing_status = MDrv_SC_Bouncing_Status(u32MotionValue);
...
BYTE MApi_XC_DLC_GetAverageValue(void)
{
    WORD wTotalPixCnt, wTotalPixWeight;
    wTotalPixCnt =  g_u16Total_Count[_WinIdx];
    wTotalPixWeight = g_u16Total_Weight[_WinIdx];

    WORD uwTmpAvr;

    // Normalize
    if (wTotalPixCnt!=0)
        uwTmpAvr = (256 * (DWORD)wTotalPixWeight + (wTotalPixCnt/2)) / wTotalPixCnt;
    else
        uwTmpAvr = 0;

    if (uwTmpAvr >= 255)
        uwTmpAvr = 255;
    //printf("DLC Average Value = %d\n", uwTmpAvr);
...
void MDrv_SC_NEW_SDHD_DETECT_driver(void)
{
    ...
    static MS_U16 HD_ConfirmCnt = 0x0000; // 0x7FFF == 0
...
#define ENABLE_SCALER_AUTO_VC_DRIVER         0x100
...
/******************************************************************************/
///ScenceChange and Source Change Driver
/******************************************************************************/
#define ScenceChange_NR_TH        0x2000
#define ScenceChange_VC_TH         0x1000
MS_U16 MDrv_SC_SceneChangeY( const  XC_LumaHistogramStates lumaHistogram )
{
    //XC_DLC_ApiStatus DLC_Status;
    //MApi_XC_DLC_GetStatus_Ex(&DLC_Status,MAIN_WINDOW);
    static MS_U16 Pre_LumaHistogram[32];
    MS_U16 Max_HistogramDiff = 0;
    ...
    return Max_HistogramDiff;
}

MS_U8 isSourceChange(void)
{
    MS_U8 sourceChangeInitReg;

    sourceChangeInitReg = MApi_XC_R2BYTEMSK(REG_SC_BK22_7E_L, 0x0001);
...
///VC Driver
/******************************************************************************/
void MDrv_SC_VC_DBG_SceneChange( const MS_BOOL isSceneChangeY )
{
    if( isSceneChangeY )
        MApi_XC_W2BYTEMSK(REG_SC_BK30_40_L, 0x0006, 0x0007);
    else
        MApi_XC_W2BYTEMSK(REG_SC_BK30_40_L, 0x0001, 0x0007);
}

void MDrv_SC_VC_DBG_VBurst( const MS_BOOL isVBurst )
{
    if( isVBurst )
        MApi_XC_W2BYTEMSK(REG_SC_BK30_40_L, 0x0060, 0x0070);
    else
        MApi_XC_W2BYTEMSK(REG_SC_BK30_40_L, 0x0010, 0x0070);
}

void MDrv_SC_VC_DBG_VCEn( const MS_BOOL isVCEn )
{
    if( isVCEn )
        MApi_XC_W2BYTEMSK(REG_SC_BK30_41_L, 0x0002, 0x0007);
    else
        MApi_XC_W2BYTEMSK(REG_SC_BK30_41_L, 0x0001, 0x0007);
}

void MDrv_SC_VC_TrunCnt( MS_U32 *cntR, MS_U32 *cntG, MS_U32 *cntB, MS_U32 *maxCnt )
{
	static MS_U32 maxCntIIR = 0;
	static MS_U32 trunCntRIIR = 0;
	static MS_U32 trunCntGIIR = 0;
	static MS_U32 trunCntBIIR = 0;

	MS_U32 trunCntR = ( (MS_U32)MApi_XC_R2BYTEMSK( REG_SC_BK08_53_L, 0x000F ) << 16 ) + (MS_U32)MApi_XC_R2BYTE( REG_SC_BK08_52_L );
	MS_U32 trunCntG = ( (MS_U32)MApi_XC_R2BYTEMSK( REG_SC_BK08_55_L, 0x000F ) << 16 ) + (MS_U32)MApi_XC_R2BYTE( REG_SC_BK08_54_L );
	MS_U32 trunCntB = ( (MS_U32)MApi_XC_R2BYTEMSK( REG_SC_BK08_57_L, 0x000F ) << 16 ) + (MS_U32)MApi_XC_R2BYTE( REG_SC_BK08_56_L );

	MS_U32 maxTrunCnt = trunCntR > trunCntG ? trunCntR : trunCntG;
	maxTrunCnt = maxTrunCnt > trunCntB ? maxTrunCnt : trunCntB;

	*cntR = trunCntRIIR = ( trunCntRIIR*3 + trunCntR ) >> 2;
	*cntG = trunCntGIIR = ( trunCntGIIR*3 + trunCntG ) >> 2;
	*cntB = trunCntBIIR = ( trunCntBIIR*3 + trunCntB ) >> 2;
	*maxCnt = maxCntIIR = ( maxCntIIR*3 + maxTrunCnt ) >> 2;
}

void MDrv_SC_Auto_VC_Control( const MS_U32 hpfHdTolerantReport, const MS_U32 isSceneChangeY, const MS_BOOL sourceChange )
{
	static MS_BOOL isVCEn = true;//false;
	static MS_BOOL isVCBypass = false;
	static MS_U8 bypassCnt = 0;
	static MS_U8 jumpBackCnt = 0;
	static MS_U16 OriMADi = 0;
	static MS_BOOL isMADiModeRead = false;

	MS_BOOL dbgEn = MApi_XC_R2BYTEMSK(REG_SC_BK30_38_L, 0x0001 ) & 0x1;

	if( dbgEn )
	{
		MApi_XC_W2BYTEMSK(REG_SC_BK30_3A_L, 0x0003, 0x0003 );
	}

	MDrv_SC_VC_DBG_SceneChange( isSceneChangeY );

	//MS_U8 chVCEn =  (MS_U8)MApi_XC_R2BYTEMSK( REG_SC_BK08_01_L, 0x0001);

	if( sourceChange )
	{
		isVCEn = !( MApi_XC_R2BYTEMSK( REG_SC_BK08_01_L, 0x0001 ) == 0x1 );
		isVCBypass = !isVCEn;
		bypassCnt = 0;
		jumpBackCnt = 0;
        	...
        	if( !isMADiModeRead )
        	{
	       	OriMADi = MApi_XC_R2BYTEMSK(REG_SC_BK12_02_L, 0x03FF);
            		isMADiModeRead = true;
	    	}

	        if(isVCEn)
	        {
	            MApi_XC_W2BYTEMSK(REG_SC_BK12_02_L, 0x03AA, 0x03FF );
	        }
	        else
	        {
	            MApi_XC_W2BYTEMSK(REG_SC_BK12_02_L, OriMADi, 0x03FF );
	        }
	}
	else if( isVCEn )
	{

		isMADiModeRead = false;
		if( isSceneChangeY )
		{
			MApi_XC_W2BYTEMSK(REG_SC_BK08_01_L, 0x0000, 0x0003 ); // enable VC
			//if (chVCEn == 1)
			//{
				//printf("VC[On] : src change\n");
			//}
			isVCBypass = false;
			bypassCnt = 0;
			jumpBackCnt = 0;
		...
		{
			MS_U32 trunCntR, trunCntG, trunCntB, maxTrunCnt;
			MDrv_SC_VC_TrunCnt( &trunCntR, &trunCntG, &trunCntB, &maxTrunCnt );

			MS_U8 maxLuma = MApi_XC_R2BYTEMSK(REG_SC_BK1A_0B_L, 0x00FF);
			MS_U8 minLuma = MApi_XC_R2BYTEMSK(REG_SC_BK1A_0B_L, 0xFF00) >> 8;
			MS_U8 lumaDiff = maxLuma - minLuma;

			// set SPF vertical diff statistics threshold
			MApi_XC_W2BYTEMSK(REG_SC_BK2B_5B_L, 0x0002, 0x00FF );
			MS_U32 vDiff = ( (MS_U32)MApi_XC_R2BYTE(REG_SC_BK2B_5F_L) << 16 ) + (MS_U32)MApi_XC_R2BYTE(REG_SC_BK2B_5E_L);

			// extermely high frequency pattern
			MS_BOOL isHFPattern;
			if( lumaDiff< 2 )
				isHFPattern = (hpfHdTolerantReport > 0x60000 );
			else if( lumaDiff <= 4 )
				isHFPattern = (hpfHdTolerantReport > 0x60000 );
			else if( lumaDiff <= 7 )
				isHFPattern = (hpfHdTolerantReport > 0x60000 );
			...
			// vertical burst
			MS_BOOL isVBurst = ( vDiff < 0x10 && isHFPattern );
			MDrv_SC_VC_DBG_VBurst( isVBurst );

			// color
			MS_BOOL isChormaExist = ( MApi_XC_R2BYTE(REG_SC_BK1A_60_L) > 0x100 );

			if( isVCBypass )
			{
				if( isHFPattern )
				{
					if(jumpBackCnt > 0) --jumpBackCnt;
				}
				else if( lumaDiff == 0 && vDiff < 0x10 && isChormaExist ) // possible color burst
				{
					if(jumpBackCnt > 0) --jumpBackCnt;
				}
				else
				{
					if(jumpBackCnt < 16) ++jumpBackCnt;
				}
				...
				if( !isVCBypass ){
					...
					//{
						//printf("VC[On] : LF Ptn\n");
						//printf( "VC : cnt : %lx, lumaDiff : %d, hdsd : %lx\n", maxTrunCnt, lumaDiff, hpfHdTolerantReport );
					//}
				}
			...
			else
			{
				...
				else if( lumaDiff < 4 && vDiff < 0x10 && isChormaExist && maxTrunCnt > 0xF000 )
				{
					isVCBypass = true;
				}

				if( isVCBypass ){
					MApi_XC_W2BYTEMSK(REG_SC_BK08_01_L, 0x0003, 0x0003 ); // disable VC

					//if (chVCEn == 0)
					//{
						//printf("VC[Off] : HF Ptn\n");
						//printf( "VC : cnt : %lx, lumaDiff : %d, hdsd : %lx\n", maxTrunCnt, lumaDiff, hpfHdTolerantReport );
					//}
				}
			...
			if( dbgEn ) printf( "VC : cnt : %lx, lumaDiff : %d, hdsd : %lx\n", maxTrunCnt, lumaDiff, hpfHdTolerantReport );
    ...
    MDrv_SC_VC_DBG_VCEn( !isVCBypass );
...
//-------------------------------------------------------------------------------------------------
void Hal_PQ_AdaptiveTuning(unsigned long dwBaseAddr, BYTE ucWin)
{
    _baseAddr = dwBaseAddr;
    _WinIdx = ucWin;
    static MS_U8 u8PreviousVcnt[DISPLAY_MAX_NUMS]= {0};
    MS_U8 u8CurrentVcnt;
    MS_U8  ColorSum,MR_Offset;
    MS_U16 u16Ctrl;
    //MS_U8  u8Ctrl2;
    //MS_U8  u8Ctrl3;
    //MS_U8  u8Ctrl4;
    ...
    MS_U8 u8SDHDDReportValue;
    //XC_ApiStatus stXCStatus;
    MS_BOOL bL107_en;
    //memset(&stXCStatus, 0x00, sizeof(stXCStatus));

    //
    // define for SD2HD & Video compresstion 8 bit mode
    //
    static MS_U32 hpfHdCntReport;
    MS_U16 u16MaxHistogramDiff= 0;
    XC_LumaHistogramStates lumaHistogramStates;
    MS_U8 sourceChange = 0;

    //
    // Check VCnt
    //
    u8CurrentVcnt = MDrv_SC_read_v_cnt();
    if (u8PreviousVcnt[ucWin]== u8CurrentVcnt)
    {
        return;
    }
    else
    {
        u8PreviousVcnt[ucWin]= u8CurrentVcnt;
    }

    //
    // Get adaptive function control
    //
    u16Ctrl  = MDrv_SC_get_adaptive_ctrl();
...
#if (CHIP_ID == MST9U3)
    if (u16Ctrl & ENABLE_SCALER_AUTO_VC_DRIVER)
    {
        // source change
	 sourceChange = isSourceChange();
        lumaHistogramStates = MDrv_SC_GetLumaHistogram();
        u16MaxHistogramDiff = MDrv_SC_SceneChangeY( lumaHistogramStates );
        hpfHdCntReport = (MApi_XC_R2BYTE(REG_SC_BK02_75_L) << 16) + MApi_XC_R2BYTE(REG_SC_BK02_74_L);
	 MDrv_SC_Auto_VC_Control( hpfHdCntReport, ((u16MaxHistogramDiff > ScenceChange_VC_TH) ? true : false), sourceChange );
    }
#endif
    ...
    ///Auto DBK Driver
    /******************************************************************************/
    if ((u16Ctrl & ENABLE_SCALER_AUTO_DBK_DRIVER)||(u16Ctrl & ENABLE_SCALER_AUTO_DBK_HD_DRIVER))               //New Add for AUTO DBK Test
    {
        //if(MApi_XC_GetStatus(&stXCStatus, MAIN_WINDOW) == FALSE)
        //{
        //    printf("MApi_XC_GetStatus failed because of InitData wrong, please update header file and compile again\n");
    ...
    if (u16Ctrl & ENABLE_SCALER_T12_UCDI_DRIVER)
    {
    ...
    if (u16Ctrl & ENABLE_SCALER_MCNR_DRIVER)
    {
    ...
    if (u16Ctrl & ENABLE_SCALER_NEW_SDHD_DETECT_DRIVER)
    {
    ...
    if (u16Ctrl & ENABLE_SCALER_DEFEATHERING)
    {
        MDrv_SC_de_feathering(u32MotionValue);
    }

    if (u16Ctrl & ENABLE_SCALER_DEFLICKERING)
    {
        MDrv_SC_de_flickering(u32MotionValue);
    }

    if (u16Ctrl & ENABLE_SCALER_DEBOUNCING)
    {
    ...
    UNUSED(hpfHdCntReport);
    UNUSED(u16MaxHistogramDiff);
    UNUSED(lumaHistogramStates);
    UNUSED(sourceChange);

}

void Hal_PQ_SetStatus(SC_ApiStatus* status)
{
    memcpy(&_status, status, sizeof(SC_ApiStatus));
}
