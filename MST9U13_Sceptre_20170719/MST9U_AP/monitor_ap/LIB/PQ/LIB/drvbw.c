...
MS_U8* Open_bandwidthtable_Bin(const MS_U8 * pName)
{
    ...
    if(pFile == NULL)
    {
        BW_DBG(printf("Open Bandwidthtable.bin Failed\n"));
        return NULL;
    }
    ...
    {
        BW_DBG(printf("Read Bandwidthtable.bin Error %ld %ld\n", u32Rst, (MS_U32)size));
        free(pu8TempBuffer);
        fclose(pFile);
        return NULL;
    }
...
static void _MDrv_BW_LoadTable(MS_U8 u8TabIdx)
{
    ...
    #if(REFINE_PARTITION==0)//
    const MS_U8 bandwidthfileName[] = "/Customer/pq/Bandwidth_RegTable.bin";
    #else
    ...
    if(pu8Buffer != NULL)
    {
        ...
        BW_DBG(printf("Read Bandwidthtable.bin\n"));
    }
...
#endif
#if PQ_BW_MM_ENABLE
    if ((u16Input_HSize>=1440) && (u16Input_VSize >= 900) && _gIsMMplayer)
    {
