...
void mhal_tmds_Initial(MSCombo_TMDS_PORT_INDEX enInputPort, Bool bDual)
{
  ...
  //  msWrite2Byte(REG_1120DE + wOffset, 0x0F00);// overwrite
    ...
    if((enInputPort== MSCombo_TMDS_PORT_0) || (enInputPort== MSCombo_TMDS_PORT_1))
    {
        ...
        //msWriteByteMask(REG_11210C + wOffset, 0x01, 0x01); // switch clock detect from PM.
    }
    else if((enInputPort== MSCombo_TMDS_PORT_2) || (enInputPort== MSCombo_TMDS_PORT_3))
    {
        ...
        //msWriteByteMask(REG_1120B9 + wOffset, 0x04, 0x04); // switch clock detect from PM.
    }
    ...
    else if((enInputPort == MSCombo_TMDS_PORT_2) || (enInputPort == MSCombo_TMDS_PORT_3))
    {
...
#else
        msWriteByteMask(REG_11242B, 0x00, 0x80); // Bypass symbol lock detect in coarse tune state
        msWriteByteMask(REG_11258A, 0x00, 0x1F); // The limit of the auto EQ start value (coarse tune min value)
        msWriteByteMask(REG_11258B, 0x1F, 0x1F); // The limit of the auto EQ end value (coarse tune max value)
        msWriteByteMask(REG_112467, 0x12, 0x3F); // Start EQ strength for auto EQ scan (coarse tune start value)
        ...
        msWriteByteMask(REG_112592, 0x00, 0x20); // enable fine tune symbol lock mode
        msWriteByteMask(REG_112451, 0x00, 0x04); // disable timer mode in fine tune
        msWriteByteMask(REG_112466, 0x08, 0x08); // enable tmds trigger
        msWrite2ByteMask(REG_112452, 0x0100, 0xFFFF); // coarse tune each step time
        msWriteByteMask(REG_11244D, 0x0A, 0x3F); // fine tune settel times
 #endif       
    }
 ...
 //   _mhal_tmds_CbusPadEnable(enInputPort, TRUE);//mqsh for OSD Switch Port to HDMI2.0 Nosignal
...
void mhal_tmds_CLK_SRC_Switch(MSCombo_TMDS_PORT_INDEX enInputPort, bool bFlag)
{
    ...
    if((enInputPort== MSCombo_TMDS_PORT_0) || (enInputPort== MSCombo_TMDS_PORT_1))
    {
        msWriteByteMask(REG_11210C + wOffset, bFlag ? 0x01:0x00, 0x01); // switch clock detect from PM.
    }
    else if((enInputPort== MSCombo_TMDS_PORT_2) || (enInputPort== MSCombo_TMDS_PORT_3))
    {
...
{
    BYTE i, debounceCnt = 5;
    Bool bDEStable = FALSE;
    ...
    //printData("              === bDEStable = 0x%x",bDEStable);
    return bDEStable;
}
...
/*		
	...
	//printData("wTmpValue==%d",wTmpValue);
*/
        if (!(wTmpValue & 0x40))
        {
            isDual = FALSE;
...
printData("mhal_tmds_Set_HPD==%d", bSetHPD);
printData("enInputPort==%d", enInputPort);
