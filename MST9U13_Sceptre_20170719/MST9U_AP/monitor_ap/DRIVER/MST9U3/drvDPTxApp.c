...
msWriteByteMask( REG_112640,BIT0,BIT0);//0x112640[0] AUXTX PN Swap [Leona Only]
#else
msWriteByte( REG_112607,0x48);//0x112607, 8wC8 [15] AUXRX PN no Swap
msWriteByteMask( REG_112640,0,BIT0);//0x112640[0] AUXTX PN no Swap  [Leona Only]
#endif
#if eDPTXAuxP1PNSWAP
msWriteByte( REG_112607+0x100,0xC8);//0x112607, 8wC8 [15] AUXRX PN Swap
msWriteByteMask( REG_112640+0x100,BIT0,BIT0);//0x112640[0] AUXTX PN Swap  [Leona Only]
#else
msWriteByte( REG_112607+0x100,0x48);//0x112607, 8wC8 [15] AUXRX PN no Swap
msWriteByteMask( REG_112640+0x100,0,BIT0);//0x112640[0] AUXTX PN no Swap  [Leona Only]
#endif
#if eDPTXAuxP2PNSWAP
msWriteByte( REG_112607+0x200,0xC8);//0x112607, 8wC8 [15] AUXRX PN Swap
msWriteByteMask( REG_112640+0x200,BIT0,BIT0);//0x112640[0] AUXTX PN Swap  [Leona Only]
#else
msWriteByte( REG_112607+0x200,0x48);//0x112607, 8wC8 [15] AUXRX PN no Swap
msWriteByteMask( REG_112640+0x200,0,BIT0);//0x112640[0] AUXTX PN no Swap  [Leona Only]
#endif
#if eDPTXAuxP3PNSWAP
msWriteByte( REG_112607+0x300,0xC8);//0x112607, 8wC8 [15] AUXRX PN Swap
msWriteByteMask( REG_112640+0x300,BIT0,BIT0);//0x112640[0] AUXTX PN Swap  [Leona Only]
#else
msWriteByte( REG_112607+0x300,0x48);//0x112607, 8wC8 [15] AUXRX PN no Swap
msWriteByteMask( REG_112640+0x300,0,BIT0);//0x112640[0] AUXTX PN no Swap  [Leona Only]
#endif
// ## aux debounce clock
...
#if Panel_DE_only    
		 msWrite2Byte(REG_140866+PortNum*0x100,0x7c30); //Discard virtual BS to solve freesync loss BE issue
#else
...
// Icon[5:0]={(eFuse[5:0]+4)*X/6}-41   Fn#2  (I>63 => REG_140651[0] =1)
...
// pe_adj[2:0]={(eFuse[5:0]+4)*Y/6}-12  Fn#4  (I>7 => REG_140781[1] =1)
...
#endif     
      #if eDPTXMODOnePort
       if(DPTxTraining(PortNum,eDPTXLinkRate,eDPTXLaneCount)==FALSE)
		{
			ForceDelay1ms(10);
...
//                    printData("3333333333333333 (msReadByte(REG_1408A4))==%d", (msReadByte(REG_1408A4)));
//                    printData("3333333333333333 (BIT0<<eDPTXHPDPort0)==%d", (BIT0<<eDPTXHPDPort0));
	  	      break;
    case 0x01:			
