...
void mapi_DP_SetPowerStatus(BYTE ucPowerStatus)
{
    ...
    {
 	 #if ENABLE_DP_ShortHPD_PM	
        for(i=0; i<COMBO_IP_SUPPORT_TYPE; i++)
        {
            mdrv_DP_Set_ShortHPDFlag(i, TRUE);
            //printf("\n bHPDShortPulse[%d] = %d\n ", i ,  bHPDShortPulse[i]);
        }
        #endif
    }
    if((ucPowerStatus == DP_POWER_STATUS_DC_ON) )
    {
...
{
    mdrv_DP_SetConfiguration(ucHBR20EnableIndex, ucHBR21EnableIndex, ucDPEnableIndex, ucDisplayEnableIndex);
}
...
{
    return mdrv_DP_ClrSplitFlag(ucInputPort);
}
//**************************************************************************
//  [Function Name]:
...
{
	mdrv_DP_DPRxAudioFreeRun(ucInputPort);
}
#endif
#if(DP_HDCP2_FUNCTION_SUPPORT)
//**************************************************************************
//  [Function Name]:
//                  mapi_DP_SetHDCP2CallBackFunction()
//  [Description]
//                  
//  [Arguments]:
//
...
{
    mdrv_DP_SetHDCP2CallBackFunction(pHDCP2RxFunc);

    if(pHDCP2CertRx != NULL)
    {
        mdrv_DP_HDCP2SetCertRx(pHDCP2CertRx);
    }
}

//**************************************************************************
//  [Function Name]:
//                  mapi_DP_HDCP2TxEventProc()
//  [Description]
//                  
//  [Arguments]:
//
...
{
    if(ucDataSize <= DP_HDCP2_TX_QUEUE_SIZE)
    {
        mdrv_DP_HDCP2TxEventProc(ucPortSelect, ucMessageID, pMessageData);
    }
    else
    {
        DP_API_DPRINTF("** DP HDCP2 Tx queue size too small port %d", ucPortSelect);
    }
}
