...
BOOLEAN USB_Hub_Handle(U8 port,U32 *devspeed)
{
  ...
  //for (i=0 ; i < 3 ; i++)
  {
    ...
    if(*devspeed==1)
    {
        printf("NO support Low spped device while plug-in hub\n");
        return FALSE;
    }
#endif	
...
UINT8 flib_Host20_Enumerate (UINT8 bNormalEnumerate,UINT8 bAddress)
{
  ...
  if (sAttachDevice.sDD.bProduct != 0)
  {
	  ...
	  if(uVID==0x5E3 && uPID==0x727)
		MsOS_DelayTask(10);		//alan.yu add on 3/18/2013 for special device BIZKEY
      if (flib_Host20_Issue_Control (1,bCMD,bCMD[6], pUsbData)>0)
      {
        return(0);//Fail
      }
