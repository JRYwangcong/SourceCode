...
void _mdrv_tmds_HDMIGetPktStatus(MSCombo_TMDS_PORT_INDEX enInputPort)
{
    ...
    if( tmds_port[enInputPort].InputPacketStatus & BIT2)
    {
        tmds_port[enInputPort].InputSPDInfoReceived = 1;
    }
...
void _mdrv_tmds_CheckAudio(BYTE ucComboPort)
{
    ...
    {    
        ...
        {
            if(tmds_port[ucComboPort].AudioErrorStatus == 0)
                TmpAudStatus = TMDS_AUDIO_STATE_ON;
            else if((tmds_port[ucComboPort].AudioErrorStatus == 0x04) && (mhal_tmds_CheckAVMute((MSCombo_TMDS_PORT_INDEX)ucComboPort) == FALSE)) // AVmute 
			    TmpAudStatus = TMDS_AUDIO_STATE_ON;
            else
                TmpAudStatus = TMDS_AUDIO_STATE_OFF;
        }
...
void _mdrv_tmds_HDMIPollingProc(MSCombo_TMDS_PORT_INDEX enInputPort)
{
    ...
    if(mhal_tmds_NoInputClk(enInputPort))
    {
        ...
        mhal_tmds_SetIsSplitterDone(enInputPort, FALSE);
    }
    ...
    if(mhal_tmds_Clock_Stable(enInputPort))
    {
        ...
        if(mhal_tmds_GetIsOver3G(enInputPort))
            mhal_tmds_CheckScrambleStatus(enInputPort, false);
    ...
    {
        if(!mhal_tmds_DEChange(enInputPort))
        {
            ...
            if(tmds_port[enInputPort].InputVideoMode) //  Input is HDMI
            {
                ...
                tmds_port[enInputPort].InputSPDInfoReceived = 0;
                ...
                if(tmds_port[enInputPort].InputAVIInfoReceived)
                {
                    ...
                    stColorInfo = (ST_COMBO_COLOR_FORMAT)mhal_tmds_GetColor(enInputPort);

                    if(bForceColor[enInputPort])
                    {
                        stColorInfo.ucColorType = enColorType[enInputPort];
                    }
                    ...
                    if(tmds_port[enInputPort].input_color.ucColorType != stColorInfo.ucColorType)
                    {
                        mhal_tmds_SetIsSplitterDone(enInputPort, FALSE);
                    }

                    tmds_port[enInputPort].input_color = stColorInfo;
                    ...
                    if(mhal_tmds_GetIsSplitterDone(enInputPort) == FALSE)
                    {
                        ...
                        if(GetAVICnt[enInputPort] > GetAVI_BOUND)
                        {
                            ...
                            {
                                //mhal_tmds_Over3G_420Setting(enInputPort, FALSE);
                            ...
                            else if(tmds_port[enInputPort].input_color.ucColorType == COMBO_COLOR_FORMAT_YUV_420)
                            {
                                if(mhal_tmds_InputIsHDMI2(enInputPort))
                                {
                                    mhal_tmds_Over3G_420Setting(enInputPort, TRUE);
                                }
                ...
                if(tmds_port[enInputPort].InputSPDInfoReceived)
                {
                    tmds_port[enInputPort].SPD_PktLossCnt = 0;
                    tmds_port[enInputPort].SPD_PktLoss = FALSE;
                }
                else
                {
                    if(tmds_port[enInputPort].SPD_PktLossCnt > SPD_PKT_LOSS_BOUND)
                    {
                        tmds_port[enInputPort].SPD_PktLossCnt = 0;
                        tmds_port[enInputPort].SPD_PktLoss = TRUE;
                    }
                }
    ...
    else  //DE unstable
    {
        ...
        tmds_port[enInputPort].SplitterEnable_YUV420 = FALSE;
        mhal_tmds_SetIsSplitterDone(enInputPort, FALSE);
        mhal_tmds_Link_LockClear(enInputPort);
    }
}
...
void _mdrv_tmds_DVIDualPollingProc(MSCombo_TMDS_PORT_INDEX enInputPort)
{
	...
	static Bool bFirstLoopFlag = TRUE;
    ...
    if(mhal_tmds_DE_Stable(enInputPort))//DE stable
    {
        ...
        if(tmds_port[enInputPort].InputVideoMode == 0) //DVI mode
        {
            ...
            if(_mdrv_tmds_CheckClockValid((BYTE)enInputPort) || 
				((bFirstLoopFlag == TRUE) && (mhal_tmds_PM_Clock_Detect(enInputPort))))
            {
...
void mdrv_tmds_SetConfiguration(BYTE ucDVIEnableIndex, BYTE ucDualEnableIndex, BYTE ucHDMIEnableIndex, BYTE ucHDMI2EnableIndex, BYTE ucMHLEnableIndex, BYTE ucDisplayEnableIndex)
{
	...
	ucDisplayPort = ucDisplayEnableIndex;
...
{    
    ...
    for(enInputPort = MSCombo_TMDS_PORT_0; enInputPort <= MSCombo_TMDS_PORT_MAX; enInputPort++)
    {
        if(tmds_dvi_port & (BIT0 << enInputPort))
        {
            ...
            mhal_tmds_ClockRtermControl(enInputPort, FALSE);
            mhal_tmds_MaskComboIRQ(enInputPort, FALSE);
            mhal_tmds_Initial(enInputPort, FALSE);
            mhal_tmds_CLK_SRC_Switch(enInputPort, TRUE);
        }
        else if(tmds_dual_port & (BIT0 << enInputPort))
        {
            ...
            mhal_tmds_CLK_SRC_Switch(enInputPort, FALSE);
        }
        ...
        bForceColor[enInputPort] = FALSE;
        enColorType[enInputPort] = COMBO_COLOR_FORMAT_YUV_444;
    }
	
    for(enInputPort = MSCombo_TMDS_PORT_0; enInputPort <= MSCombo_TMDS_PORT_MAX; enInputPort++)
    {
        if(ucDisplayPort & (BIT0 << enInputPort))
        {
            if(tmds_hdmi2_port & (BIT0 << enInputPort))
            { 
                mhal_tmds_HDMI20_MUX(enInputPort);
...
//
//**************************************************************************
void mdrv_tmds_DualLink_Align(BYTE enInputPort)
{
  
    mhal_tmds_DualLink_Align((MSCombo_TMDS_PORT_INDEX)enInputPort);
}

//**************************************************************************
//  [Function Name]:
//                  mdrv_tmds_PowerControl()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void mdrv_tmds_PowerControl(BYTE enInputPort, Bool bPowerOn, Bool bCLKDET)
{
    mhal_tmds_PowerControl((MSCombo_TMDS_PORT_INDEX)enInputPort, bPowerOn, bCLKDET);
}
...
void mdrv_tmds_TimerInterrupt(void)
{
    ...
    {        
        ...
        {
            if(tmds_port[i].InputVideoMode && (tmds_port[i].AVI_PktLossCnt <= AviPktLossBound))
            {
                tmds_port[i].AVI_PktLossCnt++;
            }

            if(tmds_port[i].InputVideoMode && (tmds_port[i].SPD_PktLossCnt <= SPD_PKT_LOSS_BOUND))
            {
                tmds_port[i].SPD_PktLossCnt++;
            }

            if(tmds_port[i].InputVideoMode && (tmds_port[i].VS_PktLossCnt <= VS_PKT_LOSS_BOUND))
            {
                tmds_port[i].VS_PktLossCnt++;
            }

            if(tmds_port[i].InputVideoMode && (tmds_port[i].GC_PktLossCnt <= GCP_PKT_LOSS_BOUND))
            {
        ...
        else
        {
            switch (ucPacketType)
            {
                ...
                case COMBO_PACKET_SPD_INFOFRAME:
                    if (tmds_port[ucInputPort].SPD_PktLoss == FALSE)
                        bPktReceived = TRUE;
                break;
...
Bool mdrv_tmds_Clock_Stable(BYTE enInputPort)
{
    ...
    {
        if((tmds_port[enInputPort].DualLinkDetectFinish_flag == TRUE) && (mhal_tmds_Clock_Stable((MSCombo_TMDS_PORT_INDEX)enInputPort)))
        {
            if(mhal_tmds_IsDualLink((MSCombo_TMDS_PORT_INDEX)enInputPort))
                return mhal_tmds_Link_LockCheck((MSCombo_TMDS_PORT_INDEX)enInputPort, TRUE);
            else
...
//
//**************************************************************************
Bool mdrv_tmds_Splitter_Done(BYTE enInputPort)
{
    Bool bSplitterDone = TRUE;

    if((enInputPort == MSCombo_TMDS_PORT_2) || (enInputPort == MSCombo_TMDS_PORT_3))
    {
        if(tmds_port[enInputPort].InputVideoMode)
        {
            bSplitterDone = mhal_tmds_GetIsSplitterDone((MSCombo_TMDS_PORT_INDEX)enInputPort);
        }
    }

    return bSplitterDone;
}

//**************************************************************************
//  [Function Name]:
//                  mdrv_tmds_fix_audio_clock()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void mdrv_tmds_fix_audio_clock(MScombo_TMDS_AUD_FREQ_INDEX enAudioFS, Bool bFlag)
{
    mhal_tmds_AUPLL_Setting(enAudioFS);
    mhal_tmds_fix_audio_clock(enAudioFS, bFlag);
}
...
{
    WORD wTMDSClkValCnt;
	wTMDSClkValCnt = mhal_tmds_GetClockRate((MSCombo_TMDS_PORT_INDEX)enInputPort) / 100;
    return wTMDSClkValCnt;
}

//**************************************************************************
//  [Function Name]:
//                  mdrv_tmds_GetPixelClock_10KHz()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
WORD mdrv_tmds_GetPixelClock_10KHz(BYTE enInputPort)
{
	...
	wTMDSClkValCnt = mhal_tmds_GetClockRate((MSCombo_TMDS_PORT_INDEX)enInputPort);

    if(mhal_tmds_GetInputType((MSCombo_TMDS_PORT_INDEX)enInputPort))
    {
		enColorDepth = mhal_tmds_GetColorDepthInfo((MSCombo_TMDS_PORT_INDEX)enInputPort);
		enPixelRepetition = mhal_tmds_GetPixelRepetitionInfo((MSCombo_TMDS_PORT_INDEX)enInputPort);

		// check HDMI2.0 timing
		if(mhal_tmds_InputIsHDMI2((MSCombo_TMDS_PORT_INDEX)enInputPort))
		{
			wTMDSClkValCnt = wTMDSClkValCnt * 4;
		}
		
		// check color format YUV420
		if(tmds_port[enInputPort].input_color.ucColorType == COMBO_COLOR_FORMAT_YUV_420)
		{
	...
	else
	{
		if(tmds_port[enInputPort].port_type == MsCombo_TMDS_TYPE_DUAL)	
	    {
			if((tmds_port[enInputPort].DualLinkDetectFinish_flag == TRUE) && (tmds_port[enInputPort].duallink_flag == TRUE))
			{
			    wTMDSClkValCnt = wTMDSClkValCnt * 2;
			}
	    }
...
void mdrv_tmds_ClrSplitFlag(BYTE enInputPort)
{
    tmds_port[enInputPort].SplitterEnable_Over3G = FALSE;
    tmds_port[enInputPort].SplitterEnable_YUV420 = FALSE;
}
...
//
//**************************************************************************
TMDS_HDCP_STATE mdrv_tmds_CheckHDCPState(BYTE enInputPort)
{
    return mhal_tmds_CheckHDCPState((MSCombo_TMDS_PORT_INDEX)enInputPort);
}

//**************************************************************************
//  [Function Name]:
//                  mdrv_tmds_ForceColorFormat()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void mdrv_tmds_ForceColorFormat(BYTE enInputPort, BOOL bFlag, EN_COLOR_FORMAT_TYPE enColor)
{
    bForceColor[enInputPort] = bFlag;
    enColorType[enInputPort] = enColor;
}
