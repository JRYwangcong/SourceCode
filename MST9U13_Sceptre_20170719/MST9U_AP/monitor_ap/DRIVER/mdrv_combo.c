...
#define COMBO_DRV_DPUTSTR(str)          printMsg(str)
#define COMBO_DRV_DPRINTF(str, x)       printData(str, x)
#else
...
{	
    ...
    if(!bRomCodeDone)
    {
        ...
        {
            if (mhal_combo_HDCP2CheckRomCodeResult())
            {
                bRomCodeDone = TRUE;
                bFirstDoneFlag = TRUE;
            ...
            {
                bRomCodeDone = TRUE;
                COMBO_DRV_DPUTSTR("**Combo check HDCP22 ROM done, but Result is NG!\r\n");
            }
        }
    }
