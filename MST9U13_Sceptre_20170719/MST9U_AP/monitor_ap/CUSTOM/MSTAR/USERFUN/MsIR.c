...
extern BYTE xdata g_bIRTimeOut;
...
{
    if(!(msReadByte(REG_003DAD) & _BIT1)) // buffer not empty
    {
        g_ucIrCode = msReadByte(REG_003DA9); // read ir code
        g_bIrDetect = 1;      // inform main to print ir code
       
        irDelayNop(4);
        if(msReadByte(REG_003DAD) & _BIT0) // check repeat flag
        {
            g_bIRRepeat = 1;
        }
        else
...
void irPollingHandler(void)
{
    if(g_IRRawModeDone)
    {
        g_IRRawModeDone = 0;
	 g_bIRTimeOut=0;	
        printData("6666666666666666  g_ucIrCode==%d",g_ucIrCode);
        printData("6666666666666666  g_bIRRepeat==%d",g_bIRRepeat);
  }
...
IR_printData("g_IRRawData[0]==%x", g_IRRawData[0]);
IR_printData("g_IRRawData[1]==%x", g_IRRawData[1]);
IR_printData("g_IRRawData[2]==%x", g_IRRawData[2]);
IR_printData("g_IRRawData[3]==%x", g_IRRawData[3]);
...
void irInitialize(void)
{
    ...
    INT_FIQ_IR_ENABLE(TRUE);    //enable FIQ mask
    msWriteByteMask(REG_000407, 0x80, 0x80);    //enable GPIO as IR in
#if 1//(defined(LeXingEnMa_Project))
    msWrite2Byte(REG_003D80, 0x0BF);
#else        
...
{
    {
        ...
        {
            g_IRRawData[i] = msReadByte(REG_003DA9);
		//printData("g_IRRawData[i]==%x",g_IRRawData[i]);
            msWriteBit(REG_003DB0, TRUE,_BIT0);//Read Pulse counter + 1
            irDelayNop(1);
	     g_bIRRepeat=1;		
        }
...
IR_printData("g_bIRRepeat==%x", g_bIRRepeat);
     g_bIRRepeat=0;
        if(g_IRRawData[0]==IR_HEADER_CODE0
...
void irInterruptProcess(void)
{
    ...
    g_wIRCounter[g_ucIRSWModeCount] = ((DWORD)(msReadByte(REG_003DAC)&0x07) << 16) + ((DWORD)msReadByte(REG_003DAB) << 8) + msReadByte(REG_003DAA);
...
{
    BYTE ucAddress;
    if(!(msReadByte(REG_003D10) & _BIT0)) // buffer not empty
    {
        ucAddress = msReadByte(REG_003D0C)& 0x1F;
        if(ucAddress == IR_RC5_ADDR1 || ucAddress == IR_RC5_ADDR2)
        {
            #if(IR_MODE_SEL == IR_FULL_INT_RC5_MODE)
            g_ucIrCode = msReadByte(REG_003D0D)& 0x3F;    //? 0x3F
            #else
            g_ucIrCode = msReadByte(REG_003D0D)& 0x7F;
            #endif
            msWriteByte(REG_003D12, _BIT0);//Read Pulse counter + 1
...
void irPollingHandler(void)
{
    ...
    {
        g_IRRawModeDone = 0;
        printData("888888888888  g_ucIrCode==%d",g_ucIrCode);
    }
}
...
void irInterruptProcess(void)
{
    ...
    if(!(msReadByte(REG_003D10) & _BIT0)) // buffer not empty
    {
        ucAddress = msReadByte(REG_003D0C);
        if(ucAddress == IR_RC6_ADDR)
        {
            g_ucIrCode = msReadByte(REG_003D0D);
            ucRC6Misc = msReadByte(REG_003D0E);
            msWriteByte(REG_003D12, _BIT0);//Read Pulse counter + 1
            if(ucRC6Misc & 0x10)
                g_bIRRepeat = 1;
            else
...
void irPollingHandler(void)
{
    ...
    {
        g_IRRawModeDone = 0;
        printData("99999  g_ucIrCode==%d",g_ucIrCode);
    }
}
