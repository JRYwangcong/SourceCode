...
static void sdivkr(char *u, int len, char *v, char **rout, char *f)
{
    ...
    for(iw=0; iw<lu; ++iw)
    {
        ...
        while(str_cmp(r, v)>=0)
        {
            ...
            {
                lp=strlen(r);
                p=(char *)malloc((unsigned int)(lp+1)*sizeof(char));
                ssub(r, v, p);
                memcpy(r, p, (unsigned int)(lp+1)*sizeof(char));
                free(p);
            }
            ++kh;
        }
                                              ...
                                              U32 VCN)
{
    ...
    for(i = 1; i <= u8ReadSectorCnt; i++)
    {
        ...
        memcpy(&u16SectorLastWord,(pBuff + i*_bytesPerSector - 2),2);

        if(u16SectorLastWord == u16FixUpNum)
        {
            //*(U16 *)(pBuff + i*_bytesPerSector - 2) = *(U16*)(pBuff + u16OffsetToFixUp + i*2);
            memcpy((pBuff + i*_bytesPerSector - 2),(pBuff + u16OffsetToFixUp + i*2),2);
        }
        else
        {
            __ASSERT(0);
...
static BOOLEAN N51FS_Delete_SaveRecordToDisk(U8 fsid)
{
    ...
    for(i = 1; i <= u8ReadSectorCnt; i++)
    {
        ...
        memcpy((pBuff + u16OffsetToFixUp + i*2),(pBuff + i*_bytesPerSector - 2),2);
        //*(U16*)(pBuff + i*_bytesPerSector - 2) = u16FixUpNum;
        memcpy((pBuff + i*_bytesPerSector - 2),&u16FixUpNum,2);
    }
