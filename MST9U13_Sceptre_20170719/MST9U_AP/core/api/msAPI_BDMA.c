...
BDMA_Result _MApi_BDMA_SpiNand2MIU(MS_PHYADDR u32SrcAddr,MS_PHYADDR u32DstAddr, U32 u32Len,BDMA_SrcDev eSrcDev, BDMA_DstDev eDstDev)
{
	...
	while(u32Len > 0)
	{
		...
		NandFlashReadToCache(u32SrcAddr-ReadStartaddr);	//TBD
		FlashAddr = NandFlashAddrCal(READ_FROM_CACHE,u32SrcAddr);
		...
		{   // BDMA not whole page
			MDrv_BDMA_MultiFlashCopy2Dram((BDMA_Dev)eSrcDev,eBuffDev,FlashAddr,BDMA_BUFFER_ADR,Readsize);
...
{
	DWORD dwSectorSize = GetFlashInfo(SPI0).SectorSize;
	DWORD SPIAlignAddr;	
	DWORD u32MIUAddrTemp;
	MS_U32 ProgramSize;
	...
	while(u32Len > 0)
	{
		...
		{ 
			...
			u32MIUAddrTemp = u32SrcAddr;
			u32SrcAddr = BDMA_BUFFER_ADR;
		}
		
		{
			FlashWriteEnable();
			// PP		
			FlashOutsideModeEnable(TRUE,ProgramSize);
			FlashWritePage(SPIAlignAddr);
			MDrv_BDMA_MemCopySPI(u32SrcAddr,SPIAlignAddr,dwSectorSize,eSrcDev,eDstDev);
			drvFlashWaitSPINotBusy();
			// BDMA
			FlashOutsideModeEnable(FALSE,ProgramSize);
			FlashReadSR();
		}	
		if(bPartialProgram)
			u32SrcAddr = u32MIUAddrTemp; // restore from BDMA_BUFF
	...
	*/
	...
	while(u32Len > 0)
	{
		...
		
		{
			// outside mode
			U16 u16PageIndex = 0;
			for(;u16PageIndex<(dwBlockSize/dwPageSize);u16PageIndex++)
			{
				//ForceDelay1ms(1);
				FlashWriteEnable();
				// PP		
