...
typedef struct
{
    BOOLEAN bFlashBinFlg;   // the data of current photo from flash/dram
    BOOLEAN bJpegFlg;       // jpeg or png flg for reload
    U32 u32FlashImgAddr;    // the address of current photo in flash
    U32 u32FlashImgSize;    // the size of current photo in flash
    BOOLEAN bInitFlg;       // the flg of current handle inited
    U32 u32BuffAddr;        // the address of the buffer that in dram after decompressed
    U32 u32BuffSize;        // the size of the buffer that in dram after decompressed
    U16 u16BufImgPitch;     // the pitch of the photo after decompressed
    U16 u16BufImgWidth;     // the width of the photo after decompressed
    U16 u16BufImgHeight;    // the height of of the photo after decompressed
    U8 u8BufImgFmt;         // the format of of the photo after decompressed
} MSAPI_JPEGPNGHANDLE;
