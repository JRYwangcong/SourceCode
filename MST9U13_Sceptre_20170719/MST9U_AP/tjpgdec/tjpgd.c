...
)
{
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					...
					r = g = b = 0;
					for (ty = 0; ty < w; ty++) {	/* Accumulate RGB value in the square */
						for (tx = 0; tx < w; tx++) {							
							b += *rgb24++;
							g += *rgb24++;							
							r += *rgb24++;
	...
	if (rx < mx) {
		...
		s = d = (BYTE*)jd->workbuf;
		for (ty = 0; ty < ry; ty++) {
			for (tx = 0; tx < rx; tx++) {	/* Copy effective pixels */
				*d++ = *s++;
				*d++ = *s++;
				*d++ = *s++;
			}
...
/*------------------------------*/
extern void msAPIWinDrawLine(BYTE u8IdxWin, WORD u16PositionV, WORD u16PositionH, WORD u16PixCount, BYTE *pDataBGR, BYTE *pDataLSB);
UINT out_func (JDEC* jd, void* bitmap, JRECT* rect)
{
    ...
    for (y = rect->top; y <= rect->bottom; y++) {
        /* The src buffer has to be BGR format, not RGB format */
        msAPIWinDrawLine(dev->u8widx, y, rect->left, bws/3, src, NULL);
        src += bws; 
    }
