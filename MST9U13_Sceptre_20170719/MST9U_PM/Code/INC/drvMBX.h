...
typedef struct
{
    ...
    BYTE              u8Ctrl;
    /// @ref MBX_Class
    BYTE              u8MsgClass; //should be MBX_Class, for alignment let it be S8, gcc default sizeof(enum) is 4 bytes.
    /// Command Index defined by Apps
    BYTE              u8Index;
    /// Parameter Count
    BYTE              u8ParameterCount;
    /// Parameters, Max Number @ref MAX_MBX_PARAM_SIZE
    BYTE              u8Parameters[MAX_MBX_PARAM_SIZE];
...
typedef void (*MBX_MAIL_ARRIVE_NOTIFY_FUNC)( MBX_Msg *pMsg, Bool *pbAddToQueue);

INTERFACE MBX_Result  MDrv_MBX_Init(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs);
INTERFACE MBX_Result  MDrv_MBX_DeInit(Bool bForceDiscardPendingMsg);
INTERFACE MBX_Result  MDrv_MBX_RegisterMSG(MBX_Class eMsgClass, WORD u16MsgQueueSize);
//INTERFACE MBX_Result  MDrv_MBX_RegisterMSGWithCallBack(MBX_Class eMsgClass, WORD u16MsgQueueSize, MBX_QUEUE_STATUS_NOTIFY_FUNC notifier);
//INTERFACE MBX_Result  MDrv_MBX_RegisterMSGWithCallBack(MBX_Class eMsgClass, WORD u16MsgQueueSize, MBX_MAIL_ARRIVE_NOTIFY_FUNC notifier);
INTERFACE MBX_Result  MDrv_MBX_UnRegisterMSG(MBX_Class eMsgClass, Bool bForceDiscardMsgQueue);
...
INTERFACE MBX_Result  MDrv_MBX_RecvMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, WORD u32WaitMillSecs, WORD u32Flag);
INTERFACE MBX_Result  MDrv_MBX_CheckMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, WORD u32WaitMillSecs, WORD u32Flag);
INTERFACE MBX_Result  MDrv_MBX_SendMsgLoopback(MBX_Msg *pMsg, MBX_ROLE_ID eSrcRoleId);
INTERFACE MBX_Result  MDrv_MBX_Enable(Bool bEnable);//Enable receiving regisited messages in kernel
INTERFACE MBX_Result  MDrv_MBX_RemoveLatestMsg(void);

// For Application Set/Get Information API:
INTERFACE MBX_Result  MDrv_MBX_SetInformation(MBX_ROLE_ID eTargetRole, BYTE *pU8Info, BYTE u8Size);
INTERFACE MBX_Result  MDrv_MBX_GetInformation(MBX_ROLE_ID eTargetRole, BYTE *pU8Info, BYTE u8Size);

// For Debug & Version Control:
INTERFACE void        MDrv_MBX_SetDbgLevel(BYTE u8DbgLevel);
//INTERFACE BOOL     MDrv_MBX_GetLibVer(const MSIF_Version **ppVersion);
INTERFACE Bool     MDrv_MBX_GetInfo(MBX_DrvInfo *pMbxDrvInfo);
INTERFACE Bool     MDrv_MBX_GetStatus(MBX_DrvStatus *pMbxDrvStatus);
INTERFACE void _MDrv_MBX_MsgRecvCb(BYTE eIntNum);
INTERFACE void Init_MBX(void);
#endif  //ENABLE_MBX
#undef INTERFACE
