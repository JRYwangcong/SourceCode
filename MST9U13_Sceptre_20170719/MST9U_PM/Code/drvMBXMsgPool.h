...
typedef struct
{
    ...
    Sint s16Next;  //next item in pool;
    WORD u16Usage; //the slot is used or free;
...
typedef struct
{
    ...
    WORD u16Slots; //the total number of MsgPoolItem in the MsgPool
    WORD u16FreeSlots; //the free number of MsgPoolItem in the MsgPool
    WORD u16RegistedSlots; //the registed(resevered for registed msg class) number of MsgPoolItem in the MsgPool.
}MSGPOOL_MsgPoolInfo;

typedef struct
{
    Sint s16MsgFirst; //the first MsgPoolItem slot idx in MsgPool
    Sint s16MsgEnd; //the end MsgPoolItem slot idx in MsgPool

    Sint s16InstantMsgFirst; //the first instant MsgPoolItem slot idx in MsgPool
    Sint s16InstantMsgEnd; //the end instant MsgPoolItem slot idx in MsgPool

    WORD u16MsgNum; //the number of MsgPoolItem in MsgQ;
    WORD u16InstantMsgNum; //the number of instant MsgPoolItem in MsgQ;

    WORD u16MsgQStatus; //the MsgQ Status: invalid; normal, empty; voerflow;
    WORD u16MsgQSize; //the MsgQ size when the msg class is registed; should be no larger than MAX_MBX_QUEUE_SIZE
...
INTERFACE MSGPOOL_Result MDrv_MSGQ_DeInit(Bool bForceDiscardPendingMsg);

INTERFACE MSGPOOL_Result MDrv_MSGQ_RegisterMSG(Sint s16MsgQID, WORD u16MsgQSize);
INTERFACE MSGPOOL_Result MDrv_MSGQ_UnRegisterMSG(Sint s16MsgQID, Bool bForceDiscardMsgQueue);
INTERFACE MSGPOOL_Result MDrv_MSGQ_ClearMSG(Sint s16MsgQID);

INTERFACE MSGPOOL_Result MDrv_MSGQ_GetMsgQStatus(Sint s16MsgQID, MBX_MSGQ_Status *pMsgQStatus);

INTERFACE MSGPOOL_Result MDrv_MSGQ_RecvMsg(Sint s16MsgQIdx, MBX_Msg* pMsg, Bool bInstantMsg);
