...
// Debug Macros
//=============================================================================
#define halMBX_DEBUG    1
#if ENABLE_DEBUG&&halMBX_DEBUG
    #define halMBX_printData(str, value)   printData(str, value)
    #define halMBX_printMsg(str)           printMsg(str)
#else
    #define halMBX_printData(str, value)
    #define halMBX_printMsg(str)
#endif
/*
//#define MBXHAL_DEBUG
#ifdef MBXHAL_DEBUG
...
// Local Variables
//=============================================================================
static WORD XDATA _u16MbxGroupIdMBXHAL[E_MBX_ROLE_MAX][E_MBX_ROLE_MAX] =
                                        { {0xFF, REG_MBX_GROUP0, REG_MBX_GROUP2}, {REG_MBX_GROUP1, 0xFF, 0xFF}, {REG_MBX_GROUP3, 0xFF, 0xFF} };
static WORD XDATA _u32RIUBaseAddrMBX = 0;
//=============================================================================
// Global Variables
//=============================================================================
...
void _MHAL_MBX_FireMsg (MBX_Msg* pMbxMsg, MBX_ROLE_ID eSrcRole)
{
    ...
    MBX_WriteByte((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+REG8_MBX_MAIL_CLASS, pMbxMsg->u8MsgClass);
    MBX_WriteByte((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+REG8_MBX_MAIL_IDX, pMbxMsg->u8Index);
    MBX_WriteByte((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+REG8_MBX_PARAMETER_CNT, pMbxMsg->u8ParameterCount);

    //halMBX_printData("class 0x%x", MBX_ReadByte((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+REG8_MBX_MAIL_CLASS));
    //halMBX_printData("mbx class 0x%x", pMbxMsg->u8MsgClass);
    //halMBX_printData("idx 0x%x", MBX_ReadByte((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+REG8_MBX_MAIL_IDX));
    //halMBX_printData("mbx idx 0x%x",pMbxMsg->u8Index);
    ...
    for(s32Idx=0;  s32Idx<pMbxMsg->u8ParameterCount; s32Idx++)
    {
        ...
        MBX_WriteByte(((_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID])+s32Idx+REG8_MBX_PARAMETER_S),pMbxMsg->u8Parameters[s32Idx]);
        //halMBX_printData("====  mbx 0x%x", pMbxMsg->u8Parameters[s32Idx]);
        //halMBX_printData("====  data 0x%x", MBX_ReadByte(_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID]+s32Idx+REG8_MBX_PARAMETER_S));
    }
...
{
    Slong XDATA s32Idx;
    WORD XDATA u32Address;

    u32Address = _u16MbxGroupIdMBXHAL[pMbxMsg->eRoleID][eDstRole];
    pMbxMsg->u8Ctrl = MBX_ReadByte(u32Address+REG8_MBX_CTRL);
    pMbxMsg->u8MsgClass = MBX_ReadByte(u32Address+REG8_MBX_MAIL_CLASS);
    pMbxMsg->u8Index = MBX_ReadByte(u32Address+REG8_MBX_MAIL_IDX);
    pMbxMsg->u8ParameterCount = MBX_ReadByte(u32Address+REG8_MBX_PARAMETER_CNT);
    ...
    */
    ...
    for(s32Idx=0;  s32Idx<10; s32Idx++)
    {
        ...
        pMbxMsg->u8Parameters[s32Idx] = MBX_ReadByte(u32Address+s32Idx+REG8_MBX_PARAMETER_S);
    }
    ...
    pMbxMsg->u8S0 = MBX_ReadByte(u32Address+REG8_MBX_STATE_0);
    pMbxMsg->u8S1 = MBX_ReadByte(u32Address+REG8_MBX_STATE_1);
}
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result MHAL_MBX_Init(MBX_ROLE_ID eHostRole, WORD u32RIUBaseAddrMBX)
{
...
MBX_Result MHAL_MBX_SetConfig(MBX_ROLE_ID eHostRole)
{
    ...
    if(E_MBX_ROLE_HK == eHostRole) //It is HouseKeeping
    {
        for(s32MailIdx=0; s32MailIdx<16; s32MailIdx++)
        {
            ...
            u32Address = _u16MbxGroupIdMBXHAL[E_MBX_ROLE_HK][E_MBX_ROLE_CP];
            MBX_WriteByte(u32Address+s32MailIdx, 0x00);
            u32Address = _u16MbxGroupIdMBXHAL[E_MBX_ROLE_HK][E_MBX_ROLE_PM];
            MBX_WriteByte(u32Address+s32MailIdx, 0x00);
        }
    }
    else
    {
        for(s32MailIdx=0; s32MailIdx<16; s32MailIdx++)
        {
            ...
            u32Address = _u16MbxGroupIdMBXHAL[eHostRole][E_MBX_ROLE_HK];
            MBX_WriteByte(u32Address+s32MailIdx, 0x00);
        }
    }
...
//-------------------------------------------------------------------------------------------------
MBX_Result  MHAL_MBX_SetInformation(MBX_ROLE_ID eTargetRole, WORD u32RIUBaseAddrMBX, BYTE *pU8Info, BYTE u8Size)
{
    Slong XDATA s32Idx;
    WORD XDATA u32Address;
    _u32RIUBaseAddrMBX = u32RIUBaseAddrMBX;
    ...
    u32Address = _u16MbxGroupIdMBXHAL[eTargetRole][E_MBX_ROLE_HK];
    MBX_WriteByte(u32Address+0+REG8_MBX_PARAMETER_S, 0x00);
    MBX_WriteByte(u32Address+1+REG8_MBX_PARAMETER_S, 0x00);
    ...
    for(s32Idx=0; s32Idx<u8Size; s32Idx++)
    {
        ...
        u32Address = _u16MbxGroupIdMBXHAL[eTargetRole][E_MBX_ROLE_HK];
        MBX_WriteByte(u32Address+s32Idx+2+REG8_MBX_PARAMETER_S, pU8Info[s32Idx]);
    }
    ...
    u32Address = _u16MbxGroupIdMBXHAL[eTargetRole][E_MBX_ROLE_HK];
    MBX_WriteByte(u32Address+0+REG8_MBX_PARAMETER_S, 0x78);
    MBX_WriteByte(u32Address+1+REG8_MBX_PARAMETER_S, 0x56);

    //while((REG8_MBX_GROUP(_u16MbxGroupIdMBXHAL[eTargetRole][E_MBX_ROLE_HK], 0+REG8_MBX_PARAMETER_S) == 0x78)
            //&& (REG8_MBX_GROUP(_u16MbxGroupIdMBXHAL[eTargetRole][E_MBX_ROLE_HK], 1+REG8_MBX_PARAMETER_S) == 0x56));
    while((MBX_ReadByte(u32Address+0+REG8_MBX_PARAMETER_S) == 0x78) && (MBX_ReadByte(u32Address+1+REG8_MBX_PARAMETER_S) == 0x56))
    return E_MBX_SUCCESS;
}
...
/// <b>[MXLIB] <em>It is used for speciall need when Init-Coprocesser.</em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MHAL_MBX_GetInformation(MBX_ROLE_ID eTargetRole, WORD u32RIUBaseAddrMBX, BYTE *pU8Info, BYTE u8Size)
{
    Slong XDATA s32Idx;
    WORD XDATA u32Address;
    ...
    for(s32Idx=0; s32Idx<u8Size; s32Idx++)
    {
        ...
        pU8Info[s32Idx] = MBX_ReadByte(u32Address+s32Idx+2+REG8_MBX_PARAMETER_S);
    }
...
MBX_Result MHAL_MBX_Fire (MBX_Msg* pMbxMsg, MBX_ROLE_ID eSrcRole)
{
    ...
    /* check paramter. */
    //halMBX_printData("hal Fire eSrcRole = %x",eSrcRole);
    //halMBX_printData("hal Fire pMbxMsg->eRoleID = %x", pMbxMsg->eRoleID);
    u32Address = _u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID];
    //halMBX_printData("=====   address 0%x ====", u32Address);
    if(0xFF == u32Address)
    {
    ...
    if(bIPState > 0)
    {
        ...
        {//the mail is in process
            halMBX_printMsg("[Fire Mail Fail] : co-processor is not ready!\n");
            return  E_MBX_ERR_PEER_CPU_NOTREADY;
        }
        else
        {//mail not processed yet!
            halMBX_printMsg("[Fire Mail Fail] : co-processor is not alive!\n");
    ...
    /* fill mail box register. */
    //halMBX_printData("eSrcRole = 0x%x", eSrcRole);
    //halMBX_printData("pMbxMsg Role ID 0x%x", pMbxMsg->eRoleID);
...
*/
    ...
    //halMBX_printData("pFireStatus = %x", *pFireStatus);
    return E_MBX_SUCCESS;
}
