...
code BYTE tCOMBO_HDCP_BKSV[5] =
{
    0x89, 0x38, 0xae, 0x0d, 0xed
};

code BYTE tCOMBO_HDCP_KEY_TABLE[] =
{
...
//  Local Variables
//-------------------------------------------------------------------------------------------------
ST_COMBO_IP_PARAMETER XDATA stComboIPParameter = {FALSE, FALSE, 0, 0, 0, 0, 0, 0, 0, 0};
...
void msAPI_combo_IPConfiguration(Bool bDisplayEnable, EN_COMBO_IP_SELECT ucComboSelect, EN_INPUTPORT_TYPE ucComboType)
{
    ...
    mapi_tmds_SetConfiguration(stComboIPParameter.ucDVIEnableIndex, stComboIPParameter.ucDualEnableIndex, stComboIPParameter.ucHDMIEnableIndex, stComboIPParameter.ucDisplayEnableIndex);

#if ENABLE_DP_INPUT
    mapi_DP_SetConfiguration(stComboIPParameter.ucHBR0EnableIndex, stComboIPParameter.ucHBR1EnableIndex,stComboIPParameter.ucDPEnableIndex,stComboIPParameter.ucDisplayEnableIndex);
#endif
...
{
    ST_COMBO_COLOR_FORMAT ucColorFormat = {COMBO_COLOR_FORMAT_DEFAULT, COMBO_COLOR_RANGE_DEFAULT, COMBO_COLORIMETRY_NONE};
...
void mapi_combo_LoadHDCPKey(void)
{
	...
	mdrv_tmds_LoadKeyViaEfuse(tCOMBO_HDCP_KEY_TABLE, tCOMBO_HDCP_BKSV, EFUSE_0);
    ...
    for(ucComboSelect = COMBO_IP_SELECT_PORT0; ucComboSelect < COMBO_IP_SELECT_MASK; ucComboSelect++)
    {
        ...
        {
            case COMBO_IP_TYPE_DVI:
                mdrv_tmds_LoadHDCPKey(ucInputPort, 0x00, tCOMBO_HDCP_KEY_TABLE, tCOMBO_HDCP_BKSV);
                break;

            case COMBO_IP_TYPE_DUAL:
                mdrv_tmds_LoadHDCPKey(ucInputPort, 0x00, tCOMBO_HDCP_KEY_TABLE, tCOMBO_HDCP_BKSV);
                mdrv_tmds_LoadHDCPKey(mdrv_tmds_GetDVIDualMainPort(ucInputPort), 0x00, tCOMBO_HDCP_KEY_TABLE, tCOMBO_HDCP_BKSV);
                break;

            case COMBO_IP_TYPE_HDMI:
            case COMBO_IP_TYPE_MHL:
                mdrv_tmds_LoadHDCPKey(ucInputPort, 0x80, tCOMBO_HDCP_KEY_TABLE, tCOMBO_HDCP_BKSV);
                break;
...
#endif
#endif // end of #ifndef _MAPI_COMBO_C_
