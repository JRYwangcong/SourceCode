...
// Debug Macros
//=============================================================================
#define drvMbX_DEBUG    1
#if ENABLE_DEBUG&&drvMbX_DEBUG
    #define MBX_DBG_printData(str, value)   printData(str, value)
    #define MBX_DBG_printMsg(str)           printMsg(str)
#else
    #define MBX_DBG_printData(str, value)
    #define MBX_DBG_printMsg(str)
#endif
/*
...
// Macros
//=============================================================================
#define INVALID_MBXCLASS(mbxClass) 	((mbxClass) >= E_MBX_CLASS_NUM)
...
#define DRV_MBX_InitLock()  {\
                                    ...
                                    {\
                                        MBX_DBG_printMsg("!!!Acquire Send Mutex for MBX Failed!\n");\
                                    ...
                                    {\
                                        MBX_DBG_printMsg("!!!Acquire Recv Mutex for MBX Failed!\n");\
...
#define DRV_MBX_DeInitLock()   {\
    ...
    #define mhal_interrupt_lock()               (msWriteBit(REG_10190D, 1, BIT2))
    #define mhal_interrupt_unlock()             (msWriteBit(REG_10190D, 0, BIT2))
...
typedef struct
{
    ...
    WORD u32IntLockerMBX;
    Slong s32SendMutexMBX;
    Slong s32RecvMutexMBX;
    BYTE u8DbgLevel;
}MBX_Mgr;
...
*/
static MBX_Mgr XDATA _mgrMBX;

void Init_MBX(void)
{
    _mgrMBX.eMbxHostRole = E_MBX_ROLE_MAX;
    //d_mgrMBX.mbxQStatusNotifier =  NULL ;
    _mgrMBX.s32RefCnt = 0;
    _mgrMBX.bMsgRecvEnable = FALSE;
    _mgrMBX.bInInterruptHandler = FALSE; ////suppose two interrupt has the same priority, won't break each other.
    _mgrMBX.s32SendMutexMBX = -1;
    _mgrMBX.s32RecvMutexMBX = -1;
    _mgrMBX.u8DbgLevel = MBX_DBG_LEVEL_NONE;
}
/// Version string
/*
...
*/
static Bool XDATA _bCalledFromDriver = {FALSE};
...
MBX_Result _MDrv_MBX_InitConfig(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs);
MBX_Result  _MDrv_MBX_SetConfig(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs);

//Message Handle functions:
static MBX_Result  drv_MBX_SendMsg(MBX_Msg *pMsg, MBX_ROLE_ID eSrcRole);
static MBX_Result  drv_MBX_RecvMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, WORD u32Flag);
static void _MDrv_MBX_MsgRecvCb(BYTE eIntNum);
//=============================================================================
// Local Function
//=============================================================================
...
MBX_Result _MDrv_MBX_InitConfig(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs)
{
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_Init] Invalid HK CPU ID \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    if((eHostRole < E_MBX_ROLE_HK) || (eHostRole >= E_MBX_ROLE_MAX))
    {
        MBX_DBG_printMsg("[MDrv_MBX_Init] Invalid Host Role ID \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    if(u32TimeoutMillSecs == 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_Init] Invalid Timeout Value \n");
        return E_MBX_ERR_INVALID_PARAM;
    }
    ...
    if(E_MBX_CPU_AEON == eHKCPU)
    {
        ...
        _mgrMBX.eMbxRole2Cpu[E_MBX_ROLE_CP] = E_MBX_CPU_PM;
        _mgrMBX.eMbxCpu2Role[E_MBX_CPU_PM] = E_MBX_ROLE_CP;
    }
    else
    {
        return E_MBX_ERR_INVALID_PARAM;
...
MBX_Result  _MDrv_MBX_SetConfig(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs)
{
    MBX_Result XDATA mbxResult = E_MBX_SUCCESS;

    //Check if MBX Already Initialized:
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_SetConfig]Reference Cnt <= 0 \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    if((_mgrMBX.eMbxHostRole != eHostRole) || (_mgrMBX.eMbxRole2Cpu[E_MBX_ROLE_HK] != eHKCPU))
    {
        ...
        {
            MBX_DBG_printData("[MDrv_MBX_SetConfig] Failed to reset Interrupt! result = %d\n", mbxResult);
            return mbxResult;
        }
...
//-------------------------------------------------------------------------------------------------
MBX_Result  drv_MBX_SendMsg(MBX_Msg *pMsg, MBX_ROLE_ID eSrcRole)
{
    MBX_Result XDATA mbxResult = E_MBX_SUCCESS;
    MBXHAL_Fire_Status XDATA mbxHalFireStatus = E_MBXHAL_FIRE_DEFAULT;
    WORD XDATA u32WaitCnt = 0;
    WORD XDATA u32TimeTicket;
    g_u32MailCounter = 1;
    ...
    if(E_MBX_SUCCESS == mbxResult)
    {//fire interrupt
        mbxResult = MHAL_MBXINT_Fire(_mgrMBX.eMbxRole2Cpu[pMsg->eRoleID], _mgrMBX.eMbxRole2Cpu[eSrcRole]);
        u32TimeTicket = g_u32MailCounter;
        do
        {
            ...
            if(E_MBX_SUCCESS != mbxResult)
            {
                MBX_DBG_printData("1. SendMail To Hardware[_MDrv_MBX_SendMsg] Result: %d \n ", mbxResult);
                break;
            }
            ...
            if(u32WaitCnt >= 0x100)
            {
                //if((g_u32MailCounter - u32TimeTicket) >= _mgrMBX.u32TimeoutMillSecs)
                {
    ...
    {
        MBX_DBG_printData("2. SendMail To Hardware[_MDrv_MBX_SendMsg] Result: %d \n ", mbxResult);
    }
...
// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  drv_MBX_RecvMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, WORD u32Flag)
{
    MBX_Result XDATA mbxResult = E_MBX_ERR_INVALID_PARAM;
    Sint XDATA s16MsgQIdx;
    ...
    if(u32Flag == MBX_CHECK_ALL_MSG_CLASS)
    {
        ...
        DRV_MBX_UnLockRecv();
        MBX_DBG_printData("[_MDrv_MBX_RecvMsg]RecvMail Fail Result=%d\n ", mbxResult);
        return mbxResult;
    }
    else
    {
...
//-------------------------------------------------------------------------------------------------
void _MDrv_MBX_MsgRecvCb(BYTE eIntNum)
{
    MBX_Msg XDATA mbxMsg;
    //MBX_MSGQ_Status msgQueueStatus;
    MBX_Result XDATA mbxResult;
    Bool XDATA bAddToQueue;
    ...
    switch(eIntNum)
    {
        case E_INT_FIQ_R2_TO_8051:
            MBX_DBG_printMsg("Receive MAIL from AEON!\n");
            mbxMsg.eRoleID = _mgrMBX.eMbxCpu2Role[E_MBX_CPU_AEON];
            break;
        default:
            //MBX_DBG_printData("[_MDrv_MBX_MsgRecvCb]Wrong interrupt number called! eIntNum=%d \n", eIntNum);
    ...
    {
        MBX_DBG_printData("[_MDrv_MBX_MsgRecvCb]Recv Mail from HW Failed! Result=%d\n", mbxResult);
        _mgrMBX.bInInterruptHandler = FALSE;
        return;
    }
    ...
    {//do not enable msg receive!
        MBX_DBG_printMsg("[_MDrv_MBX_MsgRecvCb]The MBX Disabled! \n");
    ...
    */
    ...
    switch(mbxResult)
    {
        case E_MSGPOOL_ERR_NOT_INITIALIZED:
            MBX_DBG_printData("[_MDrv_MBX_MsgRecvCb]1. Add Mail To Queue Failed! Result=%d \n", mbxResult);
            MHAL_MBX_RecvEnd(mbxMsg.eRoleID, _mgrMBX.eMbxHostRole, E_MBXHAL_RECV_DISABLED);
            break;
        case E_MSGPOOL_ERR_NO_MORE_MEMORY:
            MBX_DBG_printData("[_MDrv_MBX_MsgRecvCb]2. Add Mail To Queue Failed! Result=%d \n", mbxResult);
...
/// <b>[MXLIB] <em></em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_Init(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, WORD u32TimeoutMillSecs)
{
    WORD XDATA u32RIUBaseAddr;
    MBX_Result XDATA mbxResult;
    //MBX_DBG_printData("ehkcpu %d",eHKCPU);
    //MBX_DBG_printData("eHostRole %d",eHostRole);
    if(_mgrMBX.s32RefCnt == 0)
    {
        /*
        ...
        */
        u32RIUBaseAddr = 0x000000;
        if(E_MBX_SUCCESS != _MDrv_MBX_InitConfig(eHKCPU, eHostRole, u32TimeoutMillSecs))
        {
            MBX_DBG_printMsg("MHAL_MBXINT_Init Failed! E_MBX_ERR_INVALID_PARAM!\n");
            return E_MBX_ERR_INVALID_PARAM;
        }
        ...
        mbxResult = MHAL_MBXINT_Init(_mgrMBX.eMbxRole2Cpu[eHostRole], u32RIUBaseAddr);
        if(E_MBX_SUCCESS != mbxResult)
        {
            MBX_DBG_printData("MHAL_MBXINT_Init Failed! Failed Result=%d!\n", mbxResult);
            return mbxResult;
        }
        ...
        if(E_MBX_SUCCESS != mbxResult)
        {
            MBX_DBG_printData("MDrv_MSGPOOL_Init Failed! Result: %d!\n", mbxResult);
            return mbxResult;
        }
    ...
    else
    {
        ...
        if(E_MBX_SUCCESS != mbxResult)
        {
            MBX_DBG_printData("MDrv_MBX_SetConfig when Init MBX Failed! Result: %d!\n", mbxResult);
            return mbxResult;
        }
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_DeInit(Bool bForceDiscardPendingMsg)
{
    if(_mgrMBX.s32RefCnt <=0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_DeInit]Reference Cnt Already <= 0 \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    if((--_mgrMBX.s32RefCnt) == 0)
    {
        ...
        {//still has pending msg:
            _mgrMBX.s32RefCnt++;
            MBX_DBG_printMsg("[MDrv_MBX_DeInit]MSGQ DeInit Result: E_MBX_ERR_HAS_MSG_PENDING \n");
            return E_MBX_ERR_HAS_MSG_PENDING;
        }
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_RegisterMSG(MBX_Class eMsgClass, WORD u16MsgQueueSize)
{
    MBX_Result XDATA mbxResult = E_MBX_SUCCESS;

    if(_mgrMBX.s32RefCnt <=0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_RegisterMSG]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_RegisterMSG]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    DRV_MBX_LockRecv();
    mbxResult = (MBX_Result)MDrv_MSGQ_RegisterMSG((Sint)eMsgClass, u16MsgQueueSize);
    DRV_MBX_UnLockRecv();

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_RegisterMSG]MDrv_MSGQ_RegisterMSG Failed! Result=%d \n", mbxResult);
    }
...
//-------------------------------------------------------------------------------------------------
/*
MBX_Result  MDrv_MBX_RegisterMSGWithCallBack(MBX_Class eMsgClass, WORD u16MsgQueueSize, MBX_MAIL_ARRIVE_NOTIFY_FUNC notifier)
{
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_UnRegisterMSG(MBX_Class eMsgClass, Bool bForceDiscardMsgQueue)
{
    MBX_Result XDATA mbxResult = E_MBX_SUCCESS;

    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_UnRegisterMSG]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_UnRegisterMSG]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    DRV_MBX_LockRecv();
    mbxResult = (MBX_Result)MDrv_MSGQ_UnRegisterMSG((Sint)eMsgClass, bForceDiscardMsgQueue);
    DRV_MBX_UnLockRecv();

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_UnRegisterMSG]MDrv_MSGQ_UnRegisterMSG Failed! Result=%d \n", mbxResult);
    }
...
MBX_Result  MDrv_MBX_ClearMSG(MBX_Class eMsgClass)
{
    ...
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_ClearMSG]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    if(INVALID_MBXCLASS(eMsgClass))
    {
        MBX_DBG_printMsg("[MDrv_MBX_ClearMSG]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    DRV_MBX_LockRecv();
    mbxResult = (MBX_Result)MDrv_MSGQ_ClearMSG((Sint)eMsgClass);
    DRV_MBX_UnLockRecv();

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_ClearMSG]MDrv_MSGQ_UnRegisterMSG Failed! Result=%d \n", mbxResult);
    }
...
MBX_Result  MDrv_MBX_GetMsgQueueStatus(MBX_Class eTargetClass, MBX_MSGQ_Status *pMsgQueueStatus)
{
    ...
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_GetMsgQueueStatus]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_GetMsgQueueStatus]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    DRV_MBX_LockRecv();
    mbxResult = (MBX_Result)MDrv_MSGQ_GetMsgQStatus((Sint)eTargetClass, pMsgQueueStatus);
    DRV_MBX_UnLockRecv();

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_GetMsgQueueStatus]MDrv_MSGQ_GetMsgQStatus Failed! Result=%d \n", mbxResult);
    }
...
MBX_Result  MDrv_MBX_SendMsg(MBX_Msg *pMsg)
{
    ...
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_SendMsg]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_SendMsg]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    mbxResult = drv_MBX_SendMsg(pMsg, _mgrMBX.eMbxHostRole);

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_SendMsg]Send Mail Failed! Result=%d \n", mbxResult);
    }
...
MBX_Result  MDrv_MBX_SendMsgLoopback(MBX_Msg *pMsg, MBX_ROLE_ID eSrcRoleId)
{
    ...
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_SendMsgLoopback]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    if((NULL==pMsg) || INVALID_MBXCLASS(pMsg->u8MsgClass))
    {
        MBX_DBG_printMsg("[MDrv_MBX_SendMsgLoopback]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    MBX_DBG_printData("Send mail from %d \n", pMsg->eRoleID);
    MBX_DBG_printData("Send mail to %d! \n", eSrcRoleId);

    mbxResult = drv_MBX_SendMsg(pMsg, eSrcRoleId);

    if(E_MBX_SUCCESS != mbxResult)
    {
        MBX_DBG_printData("[MDrv_MBX_SendMsgLoopback]Send Mail Failed! Result=%d \n", mbxResult);
    }
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_RecvMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, WORD u32WaitMillSecs, WORD u32Flag)
{
    MBX_Result XDATA mbxResult = E_MBX_SUCCESS;
    WORD XDATA u32Idx;
    ...
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_RecvMsg]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_RecvMsg]E_MBX_ERR_INVALID_PARAM! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }
    ...
    mbxResult = drv_MBX_RecvMsg(eTargetClass, pMsg, u32Flag);
    ...
    if((E_MBX_SUCCESS != mbxResult) && (E_MBX_ERR_NO_MORE_MSG != mbxResult))
    {
        MBX_DBG_printData("[MDrv_MBX_RecvMsg]Recv Mail Failed! Result=%d \n", mbxResult);
    }
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_Enable(Bool bEnable)
{
    if(_mgrMBX.s32RefCnt <= 0)
    {
        MBX_DBG_printMsg("[MDrv_MBX_Enable]E_MBX_ERR_NOT_INITIALIZED! \n");
        return E_MBX_ERR_NOT_INITIALIZED;
    }

    if(_mgrMBX.s32RefCnt > 1)
    {
        MBX_DBG_printMsg("[MDrv_MBX_Enable]MBX Other Host used won't take effect! \n");
        return E_MBX_SUCCESS;
    }
...
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_SetInformation(MBX_ROLE_ID eTargetRole, BYTE *pU8Info, BYTE u8Size)
{
    WORD XDATA u32RIUBaseAddr;
    ...
    {
        MBX_DBG_printMsg("[MDrv_MBX_SetInformation] Invalid Role ID \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    if(NULL == pU8Info)
    {
        MBX_DBG_printMsg("[MDrv_MBX_SetInformation]Invalid pU8Info! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    if(u8Size > MAX_MBX_INFORMATION_SIZE)
    {
        MBX_DBG_printMsg("[MDrv_MBX_SetInformation]Invalid Information Size! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }
...
//-------------------------------------------------------------------------------------------------
MBX_Result  MDrv_MBX_GetInformation(MBX_ROLE_ID eTargetRole, BYTE *pU8Info, BYTE u8Size)
{
    WORD XDATA u32RIUBaseAddr;
    ...
    if(NULL == pU8Info)
    {
        MBX_DBG_printMsg("[MDrv_MBX_GetInformation]Invalid pU8Info! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }

    if(u8Size > MAX_MBX_INFORMATION_SIZE)
    {
        MBX_DBG_printMsg("[MDrv_MBX_GetInformation]Invalid Information Size! \n");
        return E_MBX_ERR_INVALID_PARAM;
    }
...
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
void MDrv_MBX_SetDbgLevel(BYTE u8DbgLevel)
{
...
//------------------------------------------------------------------------------
/*
BOOL MDrv_MBX_GetLibVer(const MSIF_Version **ppVersion)
{
    if (!ppVersion)
    {
        return FALSE;
...
//------------------------------------------------------------------------------
Bool MDrv_MBX_GetInfo(MBX_DrvInfo *pMbxDrvInfo)
{
    if (NULL == pMbxDrvInfo)
    {
        return FALSE;
...
//------------------------------------------------------------------------------
Bool MDrv_MBX_GetStatus(MBX_DrvStatus *pMbxDrvStatus)
{
