...
/// ----App Init:
/// --------0. If Need, MApi_MBX_SetInformation(info, size) to Init & Sync. with Co-Processor
/// --------1. MApi_MBX_Init();
/// --------2. MApi_MBX_Enable(TRUE);
/// --------3. MApi_MBX_RegisterMSG(classes, queue_size);
/// ----App Run:
/// --------1. Send Mail: MApi_MBX_SendMsg(pMsg);
/// -------------Do not need to set pMsg->eRoleID/u8Ctrl/u8S0/u8S1
/// --------2. Recv Mail:
/// ------------a). MApi_MBX_GetMsgQueueStatus(class, status);
/// ------------b). while(status.stillhasMail) MApi_MBX_RecvMsg(pMsg);
/// --------3. Reset Mail Queue: MApi_MBX_ClearMSG(class);
/// ----App Exit:
/// --------1. MApi_MBX_UnRegisterMSG(class);
/// --------2. MApi_MBX_Enable(FALSE);
/// --------3. MApi_MBX_DeInit();
////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
/*
MBX_Result MApi_MBX_RegisterMSGWithCallBack(MBX_Class eMsgClass, WORD u16MsgQueueSize, MBX_MAIL_ARRIVE_NOTIFY_FUNC notifier)
{
