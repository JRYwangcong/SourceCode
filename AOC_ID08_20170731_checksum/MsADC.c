...
{ 
    mStar_WaitForDataBlanking();//20151122 //lixw
...
void msADC_SetPhaseCode_100(BYTE phasecode)
{
    phasecode=phasecode*(127-0)/100;
    _msADC_Write2BytesMask(REG_ADC_ATOP_0F_L, (WORD)phasecode, 0x03FF);
}
#endif
//**************************************************************************
...
#else
{
    ...
    mStar_WriteByte(REGBK, ucBank);        // Switch to Original bank
    #if AdjustFocusOSDValueBy1Step
    UserPrefPhase=(UserPrefPhase*100)/127;
    #endif
    return TRUE;
}
...
#if ENABLE_ADC_RESET // default set to enable	//120502 Modify
	_msADC_Write2BytesMask(REG_ADC_ATOP_07_L, BIT0, BIT0);
	_msADC_Write2BytesMask(REG_ADC_ATOP_07_L, 0, BIT0);
	Delay1ms(2);
#endif
...
static Bool _msADC_AutoAdjustAdcGain(BYTE vsyncTime)	//110527 Modify
{
    ...
    maxRGain=maxGGain=maxBGain=0x600;
    ...
    if ((atgFlags&0xFC)!=0xFC)
        return FALSE;
    #if PanelType==PanelHSM195EU_RTN01
    UserPrefAdcRedGain=rGain-170;//200;// 1;
    UserPrefAdcGreenGain=gGain-170;//200;// 1; 
    UserPrefAdcBlueGain=bGain-170;//200;// 1; 
    #else
    UserPrefAdcRedGain=rGain-20;// 1;
    UserPrefAdcGreenGain=gGain-20;// 1; 
    UserPrefAdcBlueGain=bGain-20;// 1; 
    #endif
    msADC_SetRGBGainCode(UserPrefAdcRedGain, UserPrefAdcGreenGain, UserPrefAdcBlueGain);
...
#if 0 	//111021 Modify
///////////////////////////////////////////////////////////////////////////////
/// @note           Oringal name is msADC_AdjustCalDuring
/// @details        Adjust ADC calibration during for specific reduced blanking timings
/// @param[in]   u8CalDur: pixel
///////////////////////////////////////////////////////////////////////////////
void drvAdcAdjustCalDuring(BYTE u8CalDur)
{
    BYTE ucBank = mStar_ReadByte(REGBK);//msGetCurrentBank();

     mStar_WriteByte(REGBK, REGBANK7);//msSetCurrentBank( REGBANK7 );
     _msADC_Write2BytesMask(REG_ADC_DTOP_18_L, u8CalDur, 0x00FF); // select R1 to read//mStar_WriteWordMask( BK7_30, u8CalDur, 0x00FF );

    mStar_WriteByte(REGBK,ucBank);//msSetCurrentBank( ucBank );
}
#endif
